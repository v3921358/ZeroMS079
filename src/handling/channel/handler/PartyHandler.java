...
public class PartyHandler {
    ...
    public static final void DenyPartyRequest(LittleEndianAccessor slea, MapleClient c) {
        ...
        String leaderName = slea.readMapleAsciiString();

        MapleCharacter cfrom = c.getChannelServer().getPlayerStorage().getCharacterByName(leaderName);
        if (cfrom != null) {
    ...
    public static final void PartyOperation(LittleEndianAccessor slea, MapleClient c) {
        ...
        switch (operation) {
            case 1:
                if (party == null) {
                    if (!GameConstants.isBeginnerJob(c.getPlayer().getJob()) || c.getPlayer().getLevel() >= 10) {
                        party = World.Party.createParty(partyplayer);
                        c.getPlayer().setParty(party);
                        c.getSession().write(CWvsContext.PartyPacket.partyCreated(party.getId()));
                    } else {
                        c.getSession().write(CWvsContext.PartyPacket.partyStatusMessage(10, null));
                    }
                } else {
            ...
            case 3:
                ...
                if (party == null) {
                    ...
                    if (party != null) {
                        ...
                        c.getSession().write(CWvsContext.PartyPacket.partyStatusMessage(13, null));
                    }
                } else {
                    c.getSession().write(CWvsContext.PartyPacket.partyStatusMessage(16, null));
                }
                break;
            case 4:
                if (party == null) {
                ...
                if (theCh > 0) {
                    ...
                    c.getSession().write(CWvsContext.PartyPacket.partyStatusMessage(19, null));
                }
                break;
            case 5://was5
