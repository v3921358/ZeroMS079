...
public class BuddyListHandler {
    ...
    private static final Logger logger = LogManager.getLogger(BuddyListHandler.class);
    ...
    public static final void BuddyOperation(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (mode == 1) { // add
            ...
            if (ble != null && (ble.getGroup().equals(groupName) || !ble.isVisible())) {
                c.getSession().write(BuddylistPacket.buddylistMessage((byte) 11));//11
            } else if (ble != null && ble.isVisible()) {
                ble.setGroup(groupName);
                c.getSession().write(BuddylistPacket.updateBuddylist(buddylist.getBuddies(), 7));
                ...
                try {
                    ...
                    if (charWithId != null) {
                        ...
                        if (buddyAddResult == BuddyAddResult.BUDDYLIST_FULL) {
                            c.getSession().write(BuddylistPacket.buddylistMessage((byte) 12));//12
                        } else {
                        ...
                        c.getSession().write(BuddylistPacket.buddylistMessage((byte) 15));//was15
                    }
                } catch (SQLException ex) {
                    FileoutputUtil.outputFileError(FileoutputUtil.MysqlEx_Log, ex);
                    System.err.println(ex);
