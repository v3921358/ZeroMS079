...
public class SummonHandler {
    ...
    public static void SummonAttack(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        byte numAttacked = (slea.readByte());//(byte) ((tbyte >>> 4) & 0xF);
        if (sse != null && numAttacked > sse.mobCount) {
            chr.dropMessage(5, "警告：攻击更多的怪物比召唤可以做");
        ...
        if (summonEffect == null) {
            chr.dropMessage(5, "攻击中的错误.");
            return;
        }
        boolean magic = summonEffect.getWatk() == 0;
        int maxDmg = chr.getStat().calcMaxDamage(summonEffect, chr, magic);    // thanks Darter (YungMoozi) for reporting unchecked max dmg
        for (Pair<Integer, Integer> attackEntry : allDamage) {
            int toDamage = attackEntry.right;
            ...
            if (toDamage > maxDmg) {
                World.Broadcast.broadcastGMMessage(CWvsContext.broadcastMsg(6, "[系统警告] " + chr.getName() + " (等级 " + chr.getLevel() + ") " + "召唤兽伤害异常。 " + "最高伤害 " + maxDmg + " 本次伤害 " + toDamage + " 技能ID " + summonEffect.getSourceId()));
                FileoutputUtil.logToFile("logs/Hack/召唤兽伤害异常.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家<" + chr.getLevel() + ">: " + chr.getName() + " 怪物 " + mob.getId() + " 地图: " + chr.getMapId() + " 技能代码: " + summonEffect.getSourceId() + " 最高伤害: " + maxDmg + " 本次伤害 :" + toDamage + "是否为BOSS: " + mob.getStats().isBoss());

                toDamage = (int) maxDmg;
            }
            //if (toDamage < (chr.getStat().getCurrentMaxBaseDamage() * 50.0 * (summonEffect.getSelfDestruction() + summonEffect.getDamage() + chr.getStat().getDamageIncrease(summonEffect.getSourceId())) / 100.0)) { //10 x dmg.. eh
