...
//                    System.out.println("Kind movement: " + kind + ", Remaining : " + (numCommands - res.size()) + " New type of movement ID : " + command + ", packet : " + lea.toString(true));
            ...
            if (kind == 6) {
              //  FileoutputUtil.log("logs/攻击怪物封包.log", "攻击怪物" + "(" + chr.getName() + ") 职业：" + MapleJob.getName(MapleJob.getById(chr.getJob())) + "移动类型: 0x" + HexTool.toString(command) + ", 封包: " + lea.toString(true));
            }
        }
        ...
        //    FileoutputUtil.log("logs/移动封包出错/移动封包出错.log", "循环次数[" + numCommands + "]和实际上获取的循环次数[" + res.size() + "]不符 " + "(" + chr.getName() + ") 职业：" + MapleJob.getName(MapleJob.getById(chr.getJob())) + "(" + chr.getJob() + "移动封包 剩余次数: " + (numCommands - res.size()) + "  封包: " + lea.toString(true));
