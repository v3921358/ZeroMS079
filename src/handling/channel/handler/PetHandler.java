...
public class PetHandler {
    ...
    public static final void PetCommand(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (Randomizer.nextInt(99) <= petCommand.getProbability()) {
            ...
            if (pet.getCloseness() < 30000) {
             //   int newCloseness = pet.getCloseness() + (petCommand.getIncrease() * c.getChannelServer().getTraitRate());//我注视的 为了增加下面的
                int newCloseness = pet.getCloseness() + (petCommand.getIncrease() * c.getChannelServer().getPetRate());
    ...
    public static final void MovePet(final LittleEndianAccessor slea, final MapleCharacter chr) {
        ...
        final int petId = (int) slea.readInt();
        slea.skip(8);
