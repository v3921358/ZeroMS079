...
public class ChatHandler {
    ...
    public static void GeneralChat(final String text, final byte unk, final MapleClient c, final MapleCharacter chr) {
        if (text.length() > 0 && chr != null && chr.getMap() != null && !CommandProcessor.processCommand(c, text, CommandType.NORMAL)) {
                if (!ServerConfig.Playerchat) {
                c.getSession().write(CWvsContext.broadcastMsg(1, "管理员从后台关闭了聊天功能。"));
                return;
            }
            ...
            if (chr.getCanTalk() || chr.isStaff()) {
                ...
                if ((chr.getMapId() == 910000000) && (c.getChannelServer().getMarketUseBoard())) {
                chr.setChalkboard(text);
                    }
            } else {
                c.getSession().write(CWvsContext.broadcastMsg(6, "你一直保持沉默，因此无法说话。"));
            }
        }
    }
        ...
        switch (type) {
            ...
            case 2:
                if (chr.getGuildId() <= 0) {
                 ...
                 if (ServerConfig.logs_chat) {
                    FileoutputUtil.logToFile("logs/聊天/家族聊天.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSessionIPAddress() + " 家族: " + chr.getGuildId() + " 玩家: " + chr.getName() + " 说了 :" + chattext);
                }
                ...
                if (messenger != null) {
                    ...
                    if (target != null) {
                        if (target.getMessenger() == null) {
                            ...
                            c.getSession().write(CField.messengerChat(c.getPlayer().getName() + " : " + target.getName() + " 忙碌中。"));
                        }
            ...
            case 0x06: // message
                if (messenger != null) {
                    ...
                    if (!c.getPlayer().isIntern() && text.length() >= 1000) {
                      ...
                      if (ServerConfig.logs_chat) {
                        FileoutputUtil.logToFile("logs/聊天/招待聊天.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSessionIPAddress() + " 招待: " + messenger.getId() + " " + text);
                    }
