...
public abstract class AbstractPlayerInteraction {
    ...
    public final void warpByName(final int mapid, final String chrname) {
        ...
        if (chr == null) {
            c.getPlayer().dropMessage(1, "找不到角色.");
...
//    }
    ...
    public void getItemLog(String mob, String itemmob) {
        FileoutputUtil.logToFile("logs/Data/" + mob + ".txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().getRemoteAddress().toString().split(":")[0] + " 帐号 " + c.getAccountName() + " 帐号ID " + c.getAccID() + " 角色名 " + c.getPlayer().getName() + " 角色ID " + c.getPlayer().getId() + " " + itemmob);
    }
    ...
    public void 组队征集喇叭(int lx, String msg) {
        switch (lx) {
            case 1:
                World.Broadcast.broadcastSmega(CWvsContext.broadcastMsg(11, this.c.getChannel(), "[组队征集令] : " + msg));
                break;
            case 2:
                World.Broadcast.broadcastSmega(CWvsContext.broadcastMsg(12, this.c.getChannel(), "[组队征集令]: " + msg));
                break;
            case 3:
                World.Broadcast.broadcastSmega(CWvsContext.broadcastMsg(3, this.c.getChannel(), "[组队征集令]: " + msg));
        }
    }
    ...
    public void 全服喇叭(int lx, String msg) {
        switch (lx) {
            case 1:
    ...
    public void 气泡喇叭(String 气泡喇叭) {
        getClient().getSession().write(CField.sendHint(气泡喇叭, 200, 200));
    }
    ...
    public String 万能排名(String zdm, int tal, String cname) {
        String result = "";
