...
public class EventManager {
    ...
    public void startInstance(MapleSquad squad, MapleMap map, int questID) {
        ...
        if (!squad.getLeader().isGM()) {
            if (squad.getMembers().size() < 1) { //less than 3
                squad.getLeader().dropMessage(5, "远征队中人员少于 " + squad.getType().i + " 人，无法开始远征任务。注意必须队伍中的角色在线且在同一地图。.");
                return;
            }
            if (name.equals("CWKPQ") && squad.getJobs().size() < 1) {
    ...
    public void broadcastShip(final int mapid, final int effect, final int mode) {
        getMapFactory().getMap(mapid).broadcastMessage(CField.boatPacket(effect));
    }
    ...
    public void broadcastShipEffect(final int mapid, final int effect) {
        getMapFactory().getMap(mapid).broadcastMessage(CField.setBoatState(effect));
    }

    public final void broadcastchangeMusic(final int mapid, final String songName) {
        getMapFactory().getMap(mapid).broadcastMessage(CField.musicChange(songName));
    }
    ...
    public boolean scheduleRandomEventInChannel(int chz) {
        ...
        while (t == null) {
            for (MapleEventType x : MapleEventType.values()) {
                if (Randomizer.nextInt(MapleEventType.values().length) == 0 && x != MapleEventType.ox答题 && x != MapleEventType.打瓶盖) {
                    t = x;
                    break;
                }
            }
