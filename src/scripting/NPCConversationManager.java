...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    private static final Logger logger = LogManager.getLogger(NPCConversationManager.class);

    public NPCConversationManager(MapleClient c, int npc, int questid, String npcscript, int mode, byte type, Invocable iv) {
...
//    }
    ...
    public void askAvatar(String text, int itemId, int... args) {
        if (lastMsg > -1) {
            return;
        }
        c.getSession().write(NPCPacket.getNPCTalkStyle(id, text, args, false, itemId));
        lastMsg = 7;
    }
    ...
    public void sendStyle(String text, int itemid, int styles[]) {
        if (lastMsg > -1) {
            return;
        }
        c.getSession().write(NPCPacket.getNPCTalkStyle(id, text, styles, false, itemid));
        lastMsg = 7;
    }

    public void sendSecondStyle(String text, int itemid, int styles[]) {
        if (lastMsg > -1) {
            return;
        }
        c.getSession().write(NPCPacket.getNPCTalkStyle(id, text, styles, true, itemid));
        lastMsg = 7;
    }
    ...
    public int gainGachaponItem(int id, int quantity, final String msg) {
        try {
            ...
            if (rareness > 0) {
                World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega(c.getPlayer().getName(), " : got a(n)", item, rareness));
            }
    ...
    public int useNebuliteGachapon() {
        try {
            ...
            if (grade >= 3 && grade != 5) {
                World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega(c.getPlayer().getName(), " : got a(n)", item, (byte) 0));
            }
    ...
    public void startQuest() {
        MapleQuest.getInstance(id2).start(getPlayer(), id);
    }
...
//public void teachSkill(int id, int skillevel, byte masterlevel, long expiration) {
    ...
    public void increaseGuildCapacity(boolean trueMax) {
        if (c.getPlayer().getMeso() < 5000000 && !trueMax) {
            c.getSession().write(CWvsContext.broadcastMsg(1, "你没有足够的金币来增加家族成员人数上限。"));
            return;
        }
        final int gid = c.getPlayer().getGuildId();
        if (gid <= 0) {
    ...
    public void displayLevelRanks() {
        c.getSession().write(GuildPacket.showLevelRanks(id, MapleGuildRanking.getInstance().getLevelRank()));
    }

    public void displayFameRanks() {
        c.getSession().write(GuildPacket.showFameRanks(id, MapleGuildRanking.getInstance().getFameRank()));
    }

    public void displayMesoRanks() {
        c.getSession().write(GuildPacket.showMesoRanks(id, MapleGuildRanking.getInstance().getMesoRank()));
    }

    public void displayStrRanks() {
        c.getSession().write(GuildPacket.showStrRanks(id, MapleGuildRanking.getInstance().getStrRank()));
    }

    public void displayDexRanks() {
        c.getSession().write(GuildPacket.showDexRanks(id, MapleGuildRanking.getInstance().getDexRank()));
    }

    public void displayIntRanks() {
        c.getSession().write(GuildPacket.showIntRanks(id, MapleGuildRanking.getInstance().getIntRank()));
    }

    public void displayLukRanks() {
        c.getSession().write(GuildPacket.showLukRanks(id, MapleGuildRanking.getInstance().getLukRank()));
    }
    ...
    public void openMerchantItemStore() {
        ...
        c.getSession().write(PlayerShopPacket.merchItemStore((byte) 0x22, 0, 0));
    ...
    public void openUI(byte type) {
        c.getSession().write(UIPacket.openUI(type));
    }
    ...
    public void showJettWanted() {
        try {
            ...
            Thread.sleep(1000);
            c.getSession().write(CField.environmentChange("newPirate/pendant_w", 3));
    ...
    public void showAdvanturerBoatScene() {
        try {
            ...
            c.getSession().write(CField.environmentChange("advStory/whistle", 4));
    ...
    public void showMapleLeafScene() {
        try {
            c.getSession().write(UIPacket.getDirectionStatus(true));
            c.getSession().write(CField.UIPacket.IntroEnableUI(1));
            c.getSession().write(CField.environmentChange("adventureStory/mapleLeaf/0", 3));
    ...
    public void showBeastTamerTutScene1() {
        try {
            ...
            c.getSession().write(CField.environmentChange("ShamanBTTuto/sound0", 4));
    ...
    public int[] getCanHairFace(int[] id) {
        ...
        for (int hairfaceid : id) {
            if (ii.hairfaceExists(hairfaceid)) {
                canHairFace.add(hairfaceid);
            } else {
                cantHairFace.add(hairfaceid);
            }
        }
        if (cantHairFace.size() > 0 && (c.getPlayer().isGM() || ServerConfig.logPackets)) {
            StringBuilder sb = new StringBuilder("正在读取的发型里有");
            sb.append(cantHairFace.size()).append("个发型客户端不支持显示，已经被清除：");
            for (int i = 0; i < cantHairFace.size(); i++) {
                sb.append(cantHairFace.get(i));
                if (i < cantHairFace.size() - 1) {
                    sb.append(",");
                }
            }
        ...
        for (int i = 0; i < canHairFace.size(); i++) {
            getHairFace[i] = canHairFace.get(i);
        }
        return getHairFace;
    }

    public String checkDrop(int mobId){
    return checkDrop(mobId,c.getPlayer());
    }
    
    
    
    
    public String checkDrop(int mobId,MapleCharacter c) {
        final List<MonsterDropEntry> ranks = MapleMonsterInformationProvider.getInstance().retrieveDrop(mobId);
        if (ranks != null && ranks.size() > 0) {
            ...
            for (int i = 0; i < ranks.size(); i++) {
                ...
                if (de.chance > 0 && (de.questid <= 0 || (de.questid > 0 && MapleQuest.getInstance(de.questid).getName().length() > 0))) {
                    ...
                    if (itemId == 0) { //meso
                        itemId = 4031041; //display sack of cash
                        namez = (de.Minimum * getClient().getChannelServer().getMesoRate() / 10) + " 到 " + (de.Maximum * getClient().getChannelServer().getMesoRate() / 10) + " 金币";
                    }
                    ch = de.chance * getClient().getChannelServer().getDropRate();
                    name.append((num + 1) + ") #v" + itemId + "#" + namez +  (c.isGM()?" - " + (Integer.valueOf(ch >= 999999 ? 1000000 : ch).doubleValue() / 10000.0) + "% 爆率. ":"")   + (de.questid > 0 && MapleQuest.getInstance(de.questid).getName().length() > 0 ? ("需要接受任务 " + MapleQuest.getInstance(de.questid).getName() + "") : "") + "\r\n");
                    num++;
                }
            }
            if (name.length() > 0) {
    ...
    public void getGachaponMega(String msg, Item item, int quantity) {
        World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega("[" + msg + "] ", " : 被玩家 “" + c.getPlayer().getName() + "” 幸运抽中！", item, (byte) 1, c.getPlayer().getClient().getChannel()));
    }
    ...
    public String searchItem(String search) {
        return SearchGenerator.searchItem(search);
    }
    
    public String SearchDropMobs(int itemid) {
        final MapleMonsterInformationProvider mi = MapleMonsterInformationProvider.getInstance();
        final List<Integer> mobs = mi.getMobByItem(itemid);
        if (mobs != null || mobs.isEmpty()) {
            final StringBuffer sb = new StringBuffer();
            sb.append("#i" + itemid + ":# 掉落怪物如下:\r\n#b");
    ...
    */
    ...
    public String 显示打怪伤害() {
        return DamageParse.打怪伤害();
    }
    
    public void 清理伤害记录表单() {
         DamageParse.清理伤害记录表单();
    }
