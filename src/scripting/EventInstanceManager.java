...
public class EventInstanceManager {
    ...
    public void registerPlayer(MapleCharacter chr) {
        ...
        try {
            ...
            FileoutputUtil.outputFileError("logs/异常/事件脚本异常.log", ex);
    ...
    public void playerDisconnected(final MapleCharacter chr, int idz) {
        ...
        try {
            ...
            Object byteValue = em.getIv().invokeFunction("playerDisconnected", this, chr);
            if (byteValue != null) {
                ret = (byte) Integer.parseInt(byteValue.toString());
            } else {
                ret = 0;
            }
    ...
    public void monsterKilled(final MapleCharacter chr, final MapleMonster mob) {
        ...
        try {
            ...
            Object byteValue = em.getIv().invokeFunction("monsterValue", this, mob.getId());
            int inc = Integer.parseInt(byteValue.toString());
            if (disposed || chr == null) {
                return;
            }
    ...
    public boolean check1() {
        for (MapleCharacter chr : getPlayers()) {
            if (!(chr.getLevel() >= 51 && chr.getLevel() <= 200)) {
                return false;
            }
        }
        return true;
