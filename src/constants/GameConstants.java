...
import client.inventory.MapleWeaponType2;
...
public class GameConstants {
    ...
    public static final int JAIL = 180000004, MAX_BUFFSTAT = 4;
    ...
    public static final int[] unusedNpcs = {9201142, 9201254, 9201030, 9010037, 9010038, 9010039, 9010040, 9300010, 9070004, 9070006, 9000017, 2041017, 9270075, 9000069, 9201029, 9130024, 9330072, 9133080, 9201152, 9330189, 9000154, 9000131, 9000132};
    //Unused npcs will be removed from map once you enter it.
    public static int 等级上限;
    public static int 提交数量;
    ...
    public static int maxViewRangeSq() {
        return Integer.MAX_VALUE; // 1024 * 768
    }

    public static int maxViewRangeSq_Half() {
        return Integer.MAX_VALUE; // 800 * 800
    }
...
//    }
    ...
    public static boolean isSeparatedSp(int job) {
        //if (job / 1000 == 1 && ((getTrueJobGrade(job) == 5) || getTrueJobGrade(job) != 0) || job / 1000 == 3 || job / 100 == 22 || job == 2001 || (job / 100 == 23 || job == 2002) || (job / 100 == 24 || job == 2003) || job / 1000 == 5 || (job / 100 == 27 || job == 2004) || job / 1000 == 6) {
        //    return true;
        // }
        if (isKOC(job)) {
            return true;
        ...
        return false;//isEvan(job) || isResistance(job) || isMercedes(job) || isJett(job) || isPhantom(job) || isMihile(job) || isAngelicBuster(job) || isKaiser(job) || isLuminous(job) || isHayato(job) || isKanna(job) || isDemonAvenger(job);
...
//        System.out.println(getSkillBook(skillid / 10000));
    ...
    public static boolean isBeginnerJob(final int job) {
        return (job == 0) || (job == 1) || (job == 1000) || (job == 2000) || (job == 2001) || (job == 2002) || (job == 2003) || (job == 2004) || (job == 3000) || (job == 3001) || (job == 3002) || (job == 4000) || (job == 4001) || (job == 4002) || (job == 5000) || (job == 6000) || (job == 6001);
    }
    ...
    public static boolean isFishingMap(int mapid) {
        return (mapid / 10 == 74100020) || mapid == 749050500 || mapid == 749050501 || mapid == 749050502 || mapid == 970020000 || mapid == 970020005;
    }
    ...
    public static boolean isTutorialMap(int mapid) {
        if (mapid < 100000000) { //Explorer & Cannoneer
            ...
            return true;
        } else if (mapid > 866000000) { //Beast Tamer
            return true;
        }
        return false;
    ...
    public static int isMonsterSpawn(MapleMap map) { //回傳生怪數量倍率
        ...
        if(ServerConfig.Openmultiplayermap){
            if(ServerConfig.Custommultiplayermap.equals("")){
                return 1 * ServerConfig.monsterSpawn;
            }
            String[] 地图列表 = ServerConfig.Custommultiplayermap.split("\\|");
            boolean a = false;
            
            for (int i = 0; i < 地图列表.length; ++i){
                if(map.getId() == Integer.parseInt(地图列表[i])){
                    a = true;
                }
            }
            if(a){
    ...
    public static boolean isA(int job, int basejob) {  // thanks Steve (kaito1410) for pointing out an improvement here
        int basebranch = basejob / 10;
        return (job / 10 == basebranch && job >= basejob) || (basebranch % 10 == 0 && job / 100 == basejob / 100);
    }

    public static MapleWeaponType2 getWeaponType2(int itemId) {
        int cat = (itemId / 10000) % 100;
        MapleWeaponType2[] type = {MapleWeaponType2.SWORD1H, MapleWeaponType2.GENERAL1H_SWING, MapleWeaponType2.GENERAL1H_SWING, MapleWeaponType2.DAGGER_OTHER, MapleWeaponType2.NOT_A_WEAPON, MapleWeaponType2.NOT_A_WEAPON, MapleWeaponType2.NOT_A_WEAPON, MapleWeaponType2.WAND, MapleWeaponType2.STAFF, MapleWeaponType2.NOT_A_WEAPON, MapleWeaponType2.SWORD2H, MapleWeaponType2.GENERAL2H_SWING, MapleWeaponType2.GENERAL2H_SWING, MapleWeaponType2.SPEAR_STAB, MapleWeaponType2.POLE_ARM_SWING, MapleWeaponType2.BOW, MapleWeaponType2.CROSSBOW, MapleWeaponType2.CLAW, MapleWeaponType2.KNUCKLE, MapleWeaponType2.GUN};
        if (cat < 30 || cat > 49) {
            return MapleWeaponType2.NOT_A_WEAPON;
        }
        return type[cat - 30];
    }

    public static double getRoundedUnitPrice(double unitPrice, int max) {
