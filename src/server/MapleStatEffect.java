            ...
            if (!found) {
                switch (sourceid) {
                    ...
                    case 51100002:
                        ...
                        break;
                    case 13101002:
                        ret.statups.put(MapleBuffStat.ARIANT_COSS_IMU2, ret.info.get(MapleStatInfo.x));
                        break;
                    case 22161004:
...
//                        break;
                    ...
                    case 3221002: // sharp eyes marksmen
                        ...
                        ret.statups.put(MapleBuffStat.SHARP_EYES, ret.info.get(MapleStatInfo.x) << 8 | ret.info.get(MapleStatInfo.y));
                        break;
            ...
            if (GameConstants.isBeginnerJob(sourceid / 10000)) {
                switch (sourceid % 10000) {
                    case 1087:
                        ...
                        ret.statups.put(MapleBuffStat.INDIE_PAD, Integer.valueOf(10));
                        ret.statups.put(MapleBuffStat.INDIE_MAD, Integer.valueOf(10));
                        ret.statups.put(MapleBuffStat.SPEED, Integer.valueOf(1));
                        break;
                    case 1085:
                    case 1090:
                        ret.info.put(MapleStatInfo.time, Integer.valueOf(2100000000));
                        ret.statups.put(MapleBuffStat.INDIE_PAD, Integer.valueOf(5));
                        ret.statups.put(MapleBuffStat.INDIE_MAD, Integer.valueOf(5));
                        ret.statups.put(MapleBuffStat.SPEED, Integer.valueOf(1));
                        break;
                    case 1179:
                        ret.info.put(MapleStatInfo.time, Integer.valueOf(2100000000));
                        ret.statups.put(MapleBuffStat.INDIE_PAD, Integer.valueOf(12));
                        ret.statups.put(MapleBuffStat.INDIE_MAD, Integer.valueOf(12));
                        ret.statups.put(MapleBuffStat.SPEED, Integer.valueOf(1));
                        break;
//            if (GameConstants.isBeginnerJob(sourceid / 10000)) {
//                switch (sourceid % 10000) {
...
//                        break;
    ...
    public final boolean applyTo(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final Point pos, int newDuration) {
        if (isHeal() && (applyfrom.getMapId() == ServerConfig.personalPvpMap || applyto.getMapId() == ServerConfig.partyPvpMap || applyto.getMapId() == ServerConfig.guildPvpMap)) {
        ...
        if (expinc != 0) {
            ...
            applyto.getClient().getSession().write(EffectPacket.showForeignEffect(19));
            ...
            for (int i = 0; i < use.getSlotLimit(); i++) { // impose order...
                Item item = use.getItem((byte) i);
                if (item != null) {
                    if (GameConstants.isBullet(item.getItemId()) && item.getQuantity() >= 200) {
                        MapleInventoryManipulator.removeFromSlot(applyto.getClient(), MapleInventoryType.USE, (short) i, (short) 200, false, true);
                        itemz = true;
                        break;
                    }
    ...
    public final void applyXenon_Combo(MapleCharacter applyto, int combo) {
        EnumMap stat = new EnumMap(MapleBuffStat.class);
        stat.put(MapleBuffStat.SUPPLY_SURPLUS, Integer.valueOf(combo));
        applyto.getClient().getSession().write(CWvsContext.BuffPacket.giveBuff(0, 99999, stat, this));
    }
    ...
    private void applyBuffEffect(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final int newDuration) {
        ...
        int localDuration = ServerConfig.wirelessbuff? 2100000000 : newDuration;
        if (primary) {
        ...
        switch (sourceid) {
            ...
            default:
                if (isPirateMorph()) {
                    ...
                    final int mountid,mountid2;
                    if(sourceid == 1017){
                        mountid = applyto.getClient().getPlayer().取骑宠ID();
                        mountid2 = applyto.getClient().getPlayer().取骑宠ID();
                    } else {
                        mountid = parseMountInfo(applyto, sourceid);
                        mountid2 = parseMountInfo_Pure(applyto, sourceid);
                    }
    ...
    private boolean isSpiritClaw() {
        return skill && sourceid == 4121006 || sourceid == 14111007 || sourceid == 5201008;
    }
