...
public class AramiaFireWorks {
    ...
    private void broadcastServer(final MapleCharacter c, final int itemid) {
        World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(6, itemid, "<频道 " + c.getClient().getChannel() + "> " + c.getMap().getMapName() + " : 阿乐米的 {" + MapleItemInformationProvider.getInstance().getName(itemid) + "} 节现在开始，请所有玩家赶往射手村公园！"));
    }
    ...
    private void startEvent(final MapleMap map) {
        map.startMapEffect("烟花正在燃烧!", 5120009);
