...
public final class MapleMap {
    ...
    private short decHP = 0, createMobInterval = ServerConfig.createMobInterval, top = 0, bottom = 0, left = 0, right = 0;
    ...
    private static final Logger logger = LogManager.getLogger(MapleMap.class);
    private boolean boat, docked, boatEffect = false, zakumDoor = true;
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob, final boolean instanced) {
        if (mob == null || chr == null || ChannelServer.getInstance(channel) == null || dropsDisabled || mob.dropsDisabled() || chr.getPyramidSubway() != null || ServerConfig.bandropitem) {
            return;
        }
        ...
        int maxSize = ServerConfig.Automaticcleaning;
        if (!instanced && mapobjects.get(MapleMapObjectType.ITEM).size() >= maxSize) {
            removeDrops();
           chr.dropMessage(6, "[系统提示] : 当前地图的道具数量达到 " + maxSize + " 系统已自动清理掉所有地上的物品.");
        }
        ...
        Point poss = new Point((chr.getPosition()).x, (chr.getPosition()).y);//全屏吸物坐标 
        final int petsuction = ServerConfig.petsuction;//全屏宠吸道具
        ...
        chServerrate = ChannelServer.getInstance(channel).getBossRate();
        }
        ...
        for (final MonsterDropEntry de : dropEntry) {
            ...
            if (Randomizer.nextInt(999999) < (int) (de.chance * chServerrate * chr.getDropMod() * (chr.getStat().dropBuff / 100.0) * (showdown / 100.0))) {
                ...
                if (de.itemId == 0) { // meso
                    ...
                    if(chr.haveItem(petsuction)){
                        spawnMobDrop(idrop, calcDropPos(ServerConfig.Droptheitem ? poss : pos, mob.getTruePosition()), mob, chr, droptype, de.questid);
                    } else {
        ...
        double mesoDecrease = Math.pow(0.93, mob.getStats().getExp() / 350.0);// 金币掉落段 
        if (mesoDecrease > 1.0) {
            mesoDecrease = 1.0;
        }
        ...
        if (tempmeso > 0) {
            ...
            if(chr.haveItem(petsuction)){
            spawnMobMesoDrop((int) (tempmeso * (chr.getStat().mesoBuff / 100.0) * chr.getDropMod() * cmServerrate / ServerConfig.Weakengold), calcDropPos(ServerConfig.Droptheitem ? poss : pos, mob.getPosition()), mob, chr, false, droptype);
            } else {
                spawnMobMesoDrop((int) (tempmeso * (chr.getStat().mesoBuff / 100.0) * chr.getDropMod() * cmServerrate / ServerConfig.Weakengold), calcDropPos(pos, mob.getPosition()), mob, chr, false, droptype);
        }
        ...
        for (final MonsterGlobalDropEntry de : globalEntry) {
            if (Randomizer.nextInt(999999) < de.chance && (de.continent < 0 || (de.continent < 10 && mapid / 100000000 == de.continent) || (de.continent < 100 && mapid / 10000000 == de.continent) || (de.continent < 1000 && mapid / 1000000 == de.continent))) {
                ...
                if (de.itemId == 0) {
                    ...
                    if(chr.haveItem(petsuction)){
                    spawnMobDrop(idrop, calcDropPos(ServerConfig.Droptheitem ? poss : pos, mob.getTruePosition()), mob, chr, de.onlySelf ? 0 : droptype, de.questid);
                    } else {    
    ...
    public final void killMonsterDpm(final MapleMonster monster) {
        if (monster == null) {
            return;
        }
        monster.setHp(0);
    ...
    public final void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops, final boolean second, byte animation, final int lastSkill) {//击杀怪物
        ...
        if (mobid == 8810018 && mapid == 240060200) { // Horntail
            World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(6, "经过无数次的挑战，终于击破了暗黑龙王的远征队！你们才是龙之林的真正英雄~"));
            charactersLock.readLock().lock();
            try {
            ...
            World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(6, "经过无数次的挑战，终于击破了进阶暗黑龙王的远征队！你们才是龙之林的真正英雄~"));
            charactersLock.readLock().lock();
            try {
                for (MapleCharacter c : characters) {
...
//            FileoutputUtil.log(FileoutputUtil.Horntail_Log, MapDebug_Log());
        ...
        } else if (mobid == 7130400 && mapid == 261040000) {//捉鬼怪物
            捉鬼任务线程.关闭进阶BOSS线程();
            MapleParty.捉鬼任务频道=0;
        }else if (mobid == 7130401 && mapid == 261040000) {//捉鬼怪物
            捉鬼任务线程.关闭进阶BOSS线程();
            MapleParty.捉鬼任务频道=0;
        }else if (mobid == 7130402 && mapid == 261040000) {//捉鬼怪物
            捉鬼任务线程.关闭进阶BOSS线程();
            MapleParty.捉鬼任务频道=0;
...
//            FileoutputUtil.log(FileoutputUtil.Zakum_Log, MapDebug_Log());
        ...
        if(mobid == ServerConfig.通缉怪物.怪物ID){
            if(!ServerConfig.通缉怪物.怪物击杀状态){
                ServerConfig.通缉怪物.怪物击杀状态 = true;
                
                
                if (chr.getParty() != null) {
                    ServerConfig.通缉怪物.击杀玩家名称 = chr.getName() + "带领他的队伍击杀";
                    for (MaplePartyCharacter chr1 : chr.getParty().getMembers()) {
                        if(chr.getParty().getLeader().getId() == chr1.getId()){
                            World.Broadcast.broadcastGMMessage(CWvsContext.broadcastMsg(5, "[怪物通缉] " + chr1.getName() + " 带领他的队伍击杀了通缉怪物,恭喜！！！"));
                        }
                        for (ChannelServer channel : ChannelServer.getAllInstances()) {
                            MapleCharacter ch = channel.getPlayerStorage().getCharacterById(chr1.getId());
                            if (ch != null) {
                                ch.设置累积击杀(chr.取累积击杀() + 1);
                                ch.dropMessage(6, "[怪物通缉] 恭喜您已完成击杀,当前累积击杀：" + chr.取累积击杀() + "。");
                            }
                        }
                    }
                    return;
                } else {
                    ServerConfig.通缉怪物.击杀玩家名称 = chr.getName();
                    chr.设置累积击杀(chr.取累积击杀() + 1);
                    World.Broadcast.broadcastGMMessage(CWvsContext.broadcastMsg(5, "[怪物通缉] " + chr.getName() + " 已成功击杀通缉怪物,恭喜这位勇士"));
                    chr.dropMessage(6, "[怪物通缉] 恭喜您已完成击杀,当前累积击杀：" + chr.取累积击杀() + "。");
                }
            }else {
                chr.dropMessage(6, "[怪物通缉] 被通缉怪物已被：" + ServerConfig.通缉怪物.击杀玩家名称 + " 这位勇士击杀,非常抱歉您来迟咯。");
            }
        }
        if (type != null) {
        ...
        if (ServerConfig.Strangeexplosionresistance) {
            final int Monsterdropoffset = ServerConfig.Monsterdropoffset;
            chr.modifyCSPoints(2, Monsterdropoffset);
            chr.dropMessage(5, "你从怪物身上获得[" + Monsterdropoffset + "]抵用券");
          //  chr.getClient().getSession().write(MaplePacketCreator.getTopMsg("《打怪爆点》:获得 " + 怪物掉落抵用卷数量 + " 抵用卷  !"));//，泡点经验 + 1
            chr.UpdateCash();//及时存档并更新抵卷数量
        }
        //if (ServerConfig.获取怪物掉落点卷开关()) {
        if (ServerConfig.Strangeexplosivepointvolume) {
            final int Monsterdroproll = ServerConfig.Monsterdroproll;
            chr.modifyCSPoints(1, Monsterdroproll);
            chr.dropMessage(5, "你从怪物身上获得[" + Monsterdroproll + "]点券");
           // chr.getClient().getSession().write(MaplePacketCreator.getTopMsg("《打怪爆点》:获得 " + 怪物掉落点卷数量 + " 点卷  !"));//，泡点经验 + 1
            chr.UpdateCash();//及时存档并更新点卷数量
        }
        ...
        //if (Randomizer.nextInt(100) < 25) { //kill nx
        //    chr.modifyCSPoints(1, (int) ((Randomizer.nextInt(cashz) + cashz + cashModifier) * (chr.getStat().cashBuff / 100.0) * chr.getCashMod()), true);
        // }
    }
        ...
        spawnAndAddRangedMapObject(summon, new DelayedPacketCreation() {
            ...
            public void sendPackets(MapleClient c) {
                if (summon != null && c.getPlayer() != null && (!summon.isChangedMap() || summon.getOwnerId() == c.getPlayer().getId())) {
                    ...
                    if ((summon.getSkill() == 3111002 || summon.getSkill() == 3211002 || summon.getSkill() == 13111004 || summon.getSkill() == 5211001 || summon.getSkill() == 5220002) && summon.getOwnerId() == c.getPlayer().getId()) {
                        broadcastMessage(SummonPacket.spawnSummon(summon, true));
                    } else if (summon.getSkill() != 3111002 && summon.getSkill() != 3211002 && summon.getSkill() != 13111004 && summon.getSkill() != 5211001 && summon.getSkill() != 5220002) {
                        broadcastMessage(SummonPacket.spawnSummon(summon, true));
                    }
                }
            }
    ...
    public void spawnKite(final MapleKite kite) {
        ...
        tMan.schedule(new Runnable() {
            @Override
            public void run() {
                MapleMap.this.broadcastMessage(CField.destroyKite(kite.getObjectId(), true));
                MapleMap.this.removeMapObject(kite);
            }
        }, 3600000);
    }
    ...
    public final void spawnMobMesoDrop(final int meso, final Point position, final MapleMapObject dropper, final MapleCharacter owner, final boolean playerDrop, final byte droptype) {
        ...
        if (ServerConfig.Fullscreenpetsuction && ServerConfig.Fullscreengoldcoin  && owner.getEventInstance() == null) {
            boolean 吸物状态 = false;
            int 宠物数据库ID = 0;
            if (owner.getId() == mdrop.character_ownerid) {
                for (final MaplePet pet : owner.getSummonedPets()) {
                    if (owner.getItemQuantity(ServerConfig.petsuction, false) > 0 && pet.getPetItemId() != 0) {
                ...
                if (吸物状态 && mdrop.getMeso() > 0) {
                    ...
                    InventoryHandler.removeItem_Pet(owner, mdrop, 宠物数据库ID);
                }
            }
        }
    ...
    public final void spawnMobDrop(final Item idrop, final Point dropPos, final MapleMonster mob, final MapleCharacter chr, final byte droptype, final int questid) {
        ...
        if (ServerConfig.Fullscreenpetsuction && ServerConfig.Fullscreensuction && chr.getEventInstance() == null && !mob.getStats().isBoss()) {
            boolean 吸物状态 = false;
            int 宠物数据库ID = 0;
            if (chr.getId() == mdrop.character_ownerid) {
                for (final MaplePet pet : chr.getSummonedPets()) {
                    if (chr.getItemQuantity(ServerConfig.petsuction, false) > 0 && pet.getPetItemId() != 0) {
                        宠物数据库ID = pet.getUniqueId();
                        吸物状态 = true;
                        break;
                    }
                ...
                if (吸物状态 && mdrop.getItem().getItemId() != 0 && MapleInventoryManipulator.checkSpace(chr.getClient(), mdrop.getItemId(), (int)mdrop.getItem().getQuantity(), mdrop.getItem().getOwner())) {
                    ...
                    InventoryHandler.removeItem_Pet(chr, mdrop, (int)petz);
    ...
    public final void addPlayer(final MapleCharacter chr) {
        List<MapleCharacter> players = getCharactersThreadsafe();
            ...
            if (chr.getClient().getChannel() == 1) {
                if (LoginServer.getChrPos() != null) {
                    for (Map.Entry<Integer, Pair<Integer, Integer>> ms : LoginServer.getChrPos().entrySet()) {
                        ...
                        if (ch == -1) {
                            victim.setPosition(new Point(ms.getValue().getLeft(), ms.getValue().getRight()));
                            victim.setStance(Randomizer.rand(4, 5));
                            chr.getClient().getSession().write(CField.spawnPlayerMapobject(victim));
                        }
                    }
                }
            }
        ...
        for (final MaplePet pet : chr.getSummonedPets()) {
            if (pet.getSummoned()) {
                ...
                chr.getClient().getSession().write(PetPacket.petStatUpdate(chr));
                chr.getClient().getSession().write(PetPacket.showPetUpdate(chr, pet));
            }
        }
        ...
        if (chr.getBuffedValue(MapleBuffStat.MONSTER_RIDING) != null && !GameConstants.isResistance(chr.getJob())) {
        ...
        if (ServerConfig.Overdrawingarchiving) {
        ...
        if (hasBoat() == 2 && (chr.getMapId() != 200090000 || chr.getMapId() != 200090010)) {
            chr.getClient().getSession().write((CField.boatPacket(1)));
        } else if (hasBoat() == 1 && (chr.getMapId() != 200090000 || chr.getMapId() != 200090010)) {
            chr.getClient().getSession().write(CField.boatPacket(2));
        }
        if (getBoatEffect() && (chr.getMapId() == 200090000 || chr.getMapId() == 200090010)) {
            chr.getClient().getSession().write(CField.setBoatState(1034));
    ...
    public void respawn(final boolean force, final long now) {
        ...
        if (force) { //cpq quick hack
            final int numShouldSpawn = monsterSpawn.size() * GameConstants.isMonsterSpawn(this) - spawnedMonstersOnMap.get();
            ...
            final int numShouldSpawn = (GameConstants.isForceRespawn(mapid) ? monsterSpawn.size() * GameConstants.isMonsterSpawn(this) : maxRegularSpawn * GameConstants.isMonsterSpawn(this)) - spawnedMonstersOnMap.get();
            if (numShouldSpawn > 0) {
                int spawned = 0;
                ...
                for (Spawns spawnPoint : randomSpawn) {
                    ...
                    if (spawnPoint.shouldSpawn(lastSpawnTime) || GameConstants.isForceRespawn(mapid) || (monsterSpawn.size() * GameConstants.isMonsterSpawn(this) < 10 && maxRegularSpawn * GameConstants.isMonsterSpawn(this) > monsterSpawn.size() * GameConstants.isMonsterSpawn(this) && partyBonusRate > 0)) {
                        spawnPoint.spawnMonster(this);
                        spawned++;
                    }
                    if (spawned >= numShouldSpawn) {
    ...
    public void setBoatEffect(boolean boatEffect) {
        this.boatEffect = boatEffect;
    }

    private boolean getBoatEffect() {
        return boatEffect;
    }
    ...
    public boolean isPvpMap() {
        return this.mapid == ServerConfig.personalPvpMap;
    }

    public boolean isPartyPvpMap() {
        return this.mapid == ServerConfig.partyPvpMap;
    }

    public boolean isGuildPvpMap() {
        return this.mapid == ServerConfig.guildPvpMap;
     ...
     * @return
     */
    public boolean isFishingMap() {
        return mapid == 749050500 || mapid == 749050501 || mapid == 749050502;
    }
    /**
     * 判断是否是市场地图
     *
