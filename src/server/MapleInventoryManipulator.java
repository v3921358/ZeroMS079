...
public class MapleInventoryManipulator {
    ...
    public static byte addId(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period, boolean hours, String gmLog) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if(itemId / 10000 == 207 || itemId / 10000 == 233){
                临时变量 = ii.getSlotMax(itemId);
            } else {
                临时变量 = (short) ServerConfig.Stackquantity;
            }
            final short slotMax = 临时变量;
        ...
        c.getPlayer().havePartyQuest(itemId);
        if(itemId == 5210000 || itemId == 5211060){
    ...
    public static boolean addFromDrop(final MapleClient c, Item item, final boolean show, final boolean enhance) {
        return addFromDrop(c, item, show, enhance, false);
    }
    ...
    public static boolean addFromDrop(final MapleClient c, Item item, final boolean show, final boolean enhance, final boolean isPetPickup) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            short 临时变量 = 0;
            if(item.getItemId() / 10000 == 207 || item.getItemId() / 10000 == 233){
                临时变量 = ii.getSlotMax(item.getItemId());
            } else {
                临时变量 = (short) ServerConfig.Stackquantity;
            }
            final short slotMax = 临时变量;
            ...
            if (!GameConstants.isRechargable(item.getItemId())) {
                    ...
                    while (quantity > 0) {
                        if (i.hasNext()) {
                            ...
                            if (oldQ < slotMax && item.getOwner().equals(eItem.getOwner()) && item.getExpiration() == eItem.getExpiration()) {
                                ...
                                quantity -= (newQ - oldQ);
                                eItem.setQuantity(newQ);
                                c.getSession().write(InventoryPacket.updateInventorySlot(type, eItem, !isPetPickup));
                            }
                        } else {
                            break;
                        }
                ...
                while (quantity > 0) {
                    ...
                    c.getSession().write(InventoryPacket.addInventorySlot(type, nItem, !isPetPickup));
                }
            } else {
                // Throwing Stars and Bullets - Add all into one slot regardless of quantity.
            ...
            c.getSession().write(InventoryPacket.addInventorySlot(type, item, !isPetPickup));
            if (GameConstants.isHarvesting(item.getItemId())) {
                c.getPlayer().getStat().handleProfessionTool(c.getPlayer());
            }
        } else {
        ...
        //}
        c.getPlayer().havePartyQuest(item.getItemId());
        if(item.getItemId() == 5210000 || item.getItemId() == 5211060){
            c.getPlayer().getStat().recalcLocalStats(c.getPlayer());
        }
        if (show) {
    ...
    public static void move(final MapleClient c, final MapleInventoryType type, final short src, final short dst) {
        ...
        if (c.getPlayer().isGM() || ServerConfig.logPackets) {
            c.getPlayer().dropMessage(6, "移动道具: src : " + src + " dst : " + dst + " 道具ID：" + source.getItemId());
        }
            ...
            if (type == MapleInventoryType.ETC && dst > 100 && dst % 100 != 0) {
                ...
                if (eSlot > 0) {
                    ...
                    if (dst % 100 > ee.getSlotCount() || ee.getType() != ii.getBagType(source.getItemId()) || ee.getType() <= 0) {
                        c.getPlayer().dropMessage(1, "您不能将该物品移动到背包中.");
                        c.getSession().write(CWvsContext.enableActions());
                        return;
                    } else {
                        eqIndicator = 0;
                    ...
                    c.getPlayer().dropMessage(1, "你不能把它移到那个袋子里.");
                    c.getSession().write(CWvsContext.enableActions());
                    return;
                }
            } else {
                c.getPlayer().dropMessage(1, "你不能在那里移动.");
                c.getSession().write(CWvsContext.enableActions());
                return;
            }
        }
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (GameConstants.isGMEquip(source.getItemId()) && !c.getPlayer().isStaff()) {
            c.getPlayer().dropMessage(1, "只有游戏管理员可以装备此物品.");
        ...
        //    return;
        //}
        // // 此处为异常物品检查，太过强大的物品不能被穿戴。
//        if (GameConstants.isOverPoweredEquip(c, source.getItemId(), src) && !c.getPlayer().isStaff()) {
//            c.getPlayer().dropMessage(1, "该物品似乎太过强大，请报告给管理员，如果您认为该系统与有误，请忽略此消息。");
...
//        }
        if (!c.getPlayer().isGM()) {
            if (source.getItemId() == 1112663 || source.getItemId() == 1112586) {
                c.getPlayer().dropMessage(1, "白色天使的祝福，和黑暗天使的祝福是目前不工作.");
                c.getSession().write(CWvsContext.enableActions());
                return;
            }
        }
        ...
        if (dst > -1200 && dst < -999 && !GameConstants.isEvanDragonItem(source.getItemId()) && !GameConstants.isMechanicItem(source.getItemId())) {
            c.getSession().write(CWvsContext.enableActions());
            return;
        } else if ((dst <= -1200 || (dst >= -999 && dst < -99)) && !stats.containsKey("cash") && ii.getPachinko(source.getItemId()) <= 0) {
            c.getSession().write(CWvsContext.enableActions());
            return;
                ...
                if (s.id.contains(source.getItemId())) {
                    ...
                    for (Integer i : s.id) {
                        if (theList.contains(i)) {
                            c.getPlayer().dropMessage(1, "你不能装备这个物品，因为你已经装备 " + (StringUtil.makeEnumHumanReadable(s.name())) + "。");
                            c.getSession().write(CWvsContext.enableActions());
                            return;
                        }
                    }
    ...
    public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, short quantity, final boolean npcInduced) {
        ...
        if (quantity < source.getQuantity() && !GameConstants.isRechargable(source.getItemId())) {
            ...
            } else if ((GameConstants.isPet(source.getItemId()) || ItemFlag.UNTRADABLE.check(flag)) && !GameConstants.isAnyDropMap(c.getPlayer().getMapId())) {
                c.getPlayer().getMap().disappearingItemDrop(c.getPlayer(), c.getPlayer(), target, dropPos);
            } else {
                c.getPlayer().getMap().spawnItemDrop(c.getPlayer(), c.getPlayer(), target, dropPos, true, true);
            }
    ...
    public static void removeAllByEquipOnlyId(MapleClient c, long inventoryitemid) {
        ...
        if (copyEquipItems != null) {
            ...
            String msgtext = "玩家" + c.getPlayer().getName() + " ID: " + c.getPlayer().getId() + " (等级" + c.getPlayer().getLevel() + ") 地图: " + c.getPlayer().getMapId() + " 在玩家背包中发现复制装备[" + ii.getName(copyEquipItems.getItemId()) + "]已经将其刪除。";
        ...
        if (copyEquipedItems != null) {
            ...
            String msgtext = "玩家" + c.getPlayer().getName() + " ID: " + c.getPlayer().getId() + " (等级" + c.getPlayer().getLevel() + ") 地图: " + c.getPlayer().getMapId() + " 在玩家穿戴中发现复制装备[" + ii.getName(copyEquipedItems.getItemId()) + "]已经将其刪除。";
            World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice(6, "[GM密语] " + msgtext));
            FileoutputUtil.logToFile("Hack/复制装备_已刪除.txt", msgtext + " 道具唯一ID: " + copyEquipedItems.getEquipOnlyId());
        }
        ...
        for (Item copyStorageItem : c.getPlayer().getStorage().getItems()) {
            if (copyStorageItem != null) {
                if (c.getPlayer().getStorage().removeItemByInventoryItemId(inventoryitemid)) {
                    String msgtext = "玩家" + c.getPlayer().getName() + " ID: " + c.getPlayer().getId() + " (等级" + c.getPlayer().getLevel() + ") 地图: " + c.getPlayer().getMapId() + " 在玩家穿戴中发现复制装备[" + ii.getName(copyEquipedItems.getItemId()) + "]已经将其刪除。";
                    World.Broadcast.broadcastGMMessage(CWvsContext.serverNotice(6, "[GM密语] " + msgtext));
                    FileoutputUtil.logToFile("Hack/复制装备_已刪除.txt", msgtext + " 道具唯一ID: " + copyStorageItem.getEquipOnlyId() + "\r\n");
                }
            }
        }
    }
        public static boolean removeById_fx(final MapleClient c, final MapleInventoryType type, final int itemId, final int quantity, final boolean fromDrop, final boolean consume) {
        int remremove = quantity;
        List<Item> ret = c.getPlayer().getInventory(type).listById(itemId);
