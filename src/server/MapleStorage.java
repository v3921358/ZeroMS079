...
public class MapleStorage implements Serializable {
    ...
    private static final Logger logger = LogManager.getLogger(MapleStorage.class);
    ...
    public static int create(int id) throws SQLException {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps = con.prepareStatement("INSERT INTO storages (accountid, slots, meso) VALUES (?, ?, ?)", DBConPool.RETURN_GENERATED_KEYS);
            ps.setInt(1, id);
            ps.setInt(2, 4);
            ps.setLong(3, 0);
            ps.executeUpdate();
