...
public abstract class MapleEvent {
                ...
                if (e.isRunning) {
                    for (int i : e.type.mapids) {
                        if (cserv.getEvent() == i) {
                            ...
                            e.broadcast(CWvsContext.broadcastMsg(0, "比赛将在60秒后开始!"));
                            e.broadcast(CField.getClock(60));
    ...
    public static String scheduleEvent(final MapleEventType event, final ChannelServer cserv) {
        ...
        World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(0, "活动 " + String.valueOf(event) + " 即将在频道 " + cserv.getChannel() + " 举行 , 参加指令@event 要参加的玩家请到频道 " + cserv.getChannel()));
        return "";
    }
