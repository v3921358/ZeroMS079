...
public class PlayerNPC extends MapleNPC implements MapleCharacterLook {
    ...
    private static final Logger logger = LogManager.getLogger(PlayerNPC.class);
    ...
    private void update(MapleCharacter chr, Connection con) {
    ...
    public void destroy(Connection con) {
        destroy(false, con); //just sql
    }

    public void destroy(boolean remove, Connection con) {
        try {
    ...
    public void saveToDB() {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            FileoutputUtil.outputFileError(FileoutputUtil.MysqlEx_Log, se);
