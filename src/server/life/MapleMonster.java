...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    private boolean fake = false, dropsDisabled = false, controllerHasAggro = false, controllerKnowsAboutAggro = false;
    ;
    ...
    public final void damage(final MapleCharacter from, int damage, final boolean updateAttackTime) {
        damage(from, damage, updateAttackTime, 0);
    }

    public final void damage(final MapleCharacter from, int damage, final boolean updateAttackTime, final int lastSkill) {
    ...
    private void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage, final int numExpSharers, byte pty, final byte Class_Bonus_EXP_PERCENT, final byte Premium_Bonus_EXP_PERCENT, final int lastskillID) {
        ...
        if (exp > 0) {
            ...
            if (GameConstants.isTutorialMap(attacker.getMapId())) {
                ...
                int ch_exp = ChannelServer.getInstance(map.getChannel()).getExpRate();
                //exp *= GameConstants.getExpRate(attacker.getJob(), ch_exp);
                if (attacker.getLevel() < 10) {
                    exp *= ch_exp;
                } else if (attacker.getLevel() < 69) {
                    exp *= ch_exp * ServerConfig.expRate69;
                } else if (attacker.getLevel() < 89) {
                    exp *= ch_exp * ServerConfig.expRate89;
                } else if (attacker.getLevel() < 109) {
                    exp *= ch_exp * ServerConfig.expRate109;
                } else if (attacker.getLevel() < 129) {
                    exp *= ch_exp * ServerConfig.expRate129;
                } else if (attacker.getLevel() < 139) {
                    exp *= ch_exp * ServerConfig.expRate139;
                } else if (attacker.getLevel() < 149) {
                    exp *= ch_exp * ServerConfig.expRate149;
                } else if (attacker.getLevel() < 159) {
                    exp *= ch_exp * ServerConfig.expRate159;
                } else if (attacker.getLevel() < 169) {
                    exp *= ch_exp * ServerConfig.expRate169;
                } else if (attacker.getLevel() < 179) {
                    exp *= ch_exp * ServerConfig.expRate179;
                } else if (attacker.getLevel() < 189) {
                    exp *= ch_exp * ServerConfig.expRate189;
                } else if (attacker.getLevel() < 200) {
                    exp *= ch_exp * ServerConfig.expRate200;
                } else if (attacker.getLevel() < 250) {
                    exp *= ch_exp * ServerConfig.expRate250;
                }else {
                    exp *= ch_exp;
                }
            }
...
//            exp = (int) Math.min(Integer.MAX_VALUE, exp * attacker.getEXPMod() * attacker.getStat().expBuff / 100.0 * (GameConstants.isTutorialMap(attacker.getMapId()) ? 1 : GameConstants.getExpRate(attacker.getJob(), ChannelServer.getInstance(map.getChannel()).getExpRate(attacker.getWorld()))));
            ...
            default: {
                for (final int i : toSpawn) {
                    ...
                    if (mob.getId() == 9300216) {
                        map.broadcastMessage(CField.environmentChange("Dojang/clear", 4));//was4
                        map.broadcastMessage(CField.environmentChange("dojang/end/clear", 3));//was3
                    }
                }
                break;
            }
    ...
    public final int getStatusSourceID(final MonsterStatus status) {
        if (status == MonsterStatus.POISON || status == MonsterStatus.VENOMOUS_WEAPON || status == MonsterStatus.NINJA_AMBUSH) {
            poisonsLock.readLock().lock();
            try {
    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff) {
        ...
        if (ServerConfig.Monsterstate) {
        if (from.hasGmLevel(5)) {
            String 状态 = "";
            if (status.getStati() != null) {
        ...
        if (stats.isBoss()) {
            ...
            if (checkboss && stat != (MonsterStatus.SPEED) && stat != (MonsterStatus.NINJA_AMBUSH) && stat != (MonsterStatus.WATK) && stat != (MonsterStatus.POISON) && stat != MonsterStatus.VENOMOUS_WEAPON && stat != MonsterStatus.NINJA_AMBUSH && stat != (MonsterStatus.DARKNESS) && stat != (MonsterStatus.MAGIC_CRASH)) {
                return;
            }
        ...
        if (stats.isFriendly() || isFake()) {
            if (stat == MonsterStatus.STUN || stat == MonsterStatus.SPEED || stat == MonsterStatus.POISON || stat == MonsterStatus.VENOMOUS_WEAPON || stat == MonsterStatus.NINJA_AMBUSH) {
                return;
            }
        }
        if ((stat == MonsterStatus.VENOMOUS_WEAPON || stat == MonsterStatus.NINJA_AMBUSH || stat == MonsterStatus.POISON) && eff == null) {
            return;
        }
        if (stati.containsKey(stat)) {
            cancelStatus(stat);
        }
        if (stat == MonsterStatus.POISON || stat == MonsterStatus.VENOMOUS_WEAPON || stat == MonsterStatus.NINJA_AMBUSH) {
            //int count = 0;
            poisonsLock.readLock().lock();
            try {
        ...
        if (poison && getHp() > 1) {
            ...
            status.setValue(status.getStati(), dam);
            ...
            status.setPoisonSchedule((int) (eff.getDamage() * from.getStat().calculateMaxBaseDamage(from.getStat().getTotalWatk(), from) / 100.0), from);
            ...
            status.setDotTime(duration);
            int dam = (int) Math.min(Short.MAX_VALUE, (int) (eff.getDamage() * from.getStat().calculateMaxBaseDamage(from.getStat().getTotalWatk(), from) / 100.0));
        ...
        if (reflection.size() > 0) {
            ...
            for (Entry<MonsterStatus, Integer> z : effect.entrySet()) {
                ...
                if (con != null) {
                    ...
                    con.getClient().getSession().write(MobPacket.applyMonsterStatus(this, effectz));
                } else {
    ...
    public final void doPoison(final MonsterStatusEffect status, final WeakReference<MapleCharacter> weakChr) {
        if ((status.getStati() == MonsterStatus.VENOMOUS_WEAPON || status.getStati() == MonsterStatus.POISON || status.getStati() == MonsterStatus.NINJA_AMBUSH) && poisons.size() <= 0) {
            return;
        }
        if (status.getStati() != MonsterStatus.VENOMOUS_WEAPON && status.getStati() != MonsterStatus.POISON && status.getStati() != MonsterStatus.NINJA_AMBUSH && !stati.containsKey(status.getStati())) {
            return;
        }
        if (weakChr == null) {
            return;
        ...
        if (status.getStati() == MonsterStatus.NINJA_AMBUSH) {
            if (damage > 0) {
                if (damage >= hp) {
                    hp = (int) 1;
                    cancelSingleStatus(status);
                } else {
                    hp = hp - damage;
                }
        ...
        public final void killedMob(final MapleMap map, final int baseExp, final boolean mostDamage, final int lastSkill) {
            ...
            for (final Entry<MapleCharacter, OnePartyAttacker> attacker : resolveAttackers().entrySet()) {
                ...
                for (final MapleCharacter expReceiver : expApplicable) {
                    ...
                    levelMod = expReceiver.getLevel() / addedPartyLevel;
                    iexp += (int) Math.round(((attacker.getKey().getId() == expReceiver.getId() ? 0.1 : 0.0) + levelMod) * innerBaseExp);
    ...
    public final void cancelStatus(final MonsterStatus stat) {
        ...
        if (con != null) {
            ...
            con.getClient().getSession().write(MobPacket.cancelMonsterStatus(this, mse));
        } else {
    ...
    public final void cancelSingleStatus(final MonsterStatusEffect stat) {
        ...
        if (stat.getStati() != MonsterStatus.POISON && stat.getStati() != MonsterStatus.VENOMOUS_WEAPON && stat.getStati() != MonsterStatus.NINJA_AMBUSH) {
        ...
        try {
            ...
            if (con != null) {
                ...
                con.getClient().getSession().write(MobPacket.cancelMonsterStatus(this, stat));
            } else {
