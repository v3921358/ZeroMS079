...
public class CashItemInfo {
    ...
    public CashItemInfo(int itemId, int count, int price, int sn, int expire, int gender, boolean sale, int likes, boolean iscs, int type) {
        this.itemId = itemId;
        this.count = count;
        this.price = price;
        this.sn = sn;
...
public int getOnSale() {
    ...
    public static class CashModInfo {
        ...
        public int discountPrice, mark, priority, sn, itemid, flags, period, gender, count, meso, unk_1, unk_2, unk_3, extra_flags, type;
        public boolean showUp, packagez, iscs;

        private CashItemInfo cii;

        public CashModInfo(int sn, int discount, int mark, boolean show, int itemid, int priority, boolean packagez, int period, int gender, int count, int meso, int unk_1, int unk_2, int unk_3, int extra_flags, boolean iscs, int type) {
            this.sn = sn;
            this.itemid = itemid;
        ...
        public CashItemInfo toCItem(CashItemInfo backup) {
            ...
            if (count <= 0) {
                c = (backup == null ? 1 : backup.getCount());
            } else {
                c = count;
            }
            if (meso <= 0) {
            ...
            if (gender < 0) {
                gen = (backup == null ? 2 : backup.getGender());
            } else {
                gen = gender;
            }
            if (!showUp) {
            ...
            cii = new CashItemInfo(item, c, price, sn, expire, gen, onSale, 0, iscs, type);
            return cii;
        }
    }
}
