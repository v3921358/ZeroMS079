    ...
    public void buy(MapleClient c, int item, short quantity) {
        ...
        if (MapleInventoryManipulator.checkSpace(c, newItem.getItemId(), newItem.getQuantity(), newItem.getOwner())) {
            ...
            if (gainmeso > 0) {
                ...
                if (chr != null) {
                    chr.dropMessage(-5, "道具 " + MapleItemInformationProvider.getInstance().getName(newItem.getItemId()) + " (" + perbundle + ") x " + quantity + " 已被其他玩家购买。 还剩下: " + pItem.bundles);
                }
                if (ServerConfig.logs_mrechant) {
                    FileoutputUtil.logToFile("logs/玩家操作/雇佣商人购买.txt", "\r\n 时间　[" + FileoutputUtil.NowTime() + "] IP: " + c.getSessionIPAddress() + " 玩家 " + c.getAccountName() + " " + c.getPlayer().getName() + " 从  " + getOwnerName() + " 的雇佣商人购买了" + MapleItemInformationProvider.getInstance().getName(newItem.getItemId()) + " (" + newItem.getItemId() + ") x" + quantity + " 单价为 : " + pItem.price);
                }
            } else {
                c.getPlayer().dropMessage(1, "你的金币不够。");
                c.getSession().write(CWvsContext.enableActions());
            }
        } else {
            c.getPlayer().dropMessage(1, "你的背包空间不足。");
            c.getSession().write(CWvsContext.enableActions());
        }
    }
