...
public class MapleSquad {
        ...
        while (type.queue.get(ch).size() > 0 && ChannelServer.getInstance(ch).getMapleSquad(type) == null) {
            ...
            if (theirCh > 0) {
                ...
                if (lead != null && lead.getMapId() == beginMapId && lead.getClient().getChannel() == ch) {
                    ...
                    if (lead != null) {
                        lead.dropMessage(6, "由于你没有在正确的频道和地图上，你的球队已经被忽略了.");
                    }
                    getBeginMap().broadcastMessage(CWvsContext.broadcastMsg(6, nextPlayerId + "'由于球员没有在正确的频道和地图上被跳过."));
                    type.queuedPlayers.get(ch).add(new Pair<>(nextPlayerId, "没有地图"));
                }
            } else {
                getBeginMap().broadcastMessage(CWvsContext.broadcastMsg(6, nextPlayerId + "'由于球员没有在线，球队已经被跳过了."));
                type.queuedPlayers.get(ch).add(new Pair<>(nextPlayerId, "不在线"));
            }
        }
    }
    ...
    public void clear() {
        ...
        if (ChannelServer.getInstance(ch).getMapleSquad(type) != null) {
            MapleMap map = ChannelServer.getInstance(ch).getMapFactory().getMap(211042300);
            if (!map.isZakumDoor()) {
                ChannelServer.getInstance(ch).getMapFactory().getMap(211042300).resetReactors();
            }
        }
        members.clear();
        bannedMembers.clear();
    ...
    public void acceptMember(int pos) {
        ...
        if (toadd != null && getChar(toadd) != null) {
            ...
            getChar(toadd).dropMessage(5, getLeaderName() + " 已决定将你加入球队.");
        }
    }
    ...
    public void banMember(int pos) {
        ...
        if (toban != null && getChar(toban) != null) {
            ...
            getChar(toban).dropMessage(5, getLeaderName() + " 已将你从名单中移除.");
        }
    }
    ...
    public String getSquadMemberString(byte type) {
        switch (type) {
            ...
            case 1: {
                StringBuilder sb = new StringBuilder("总共 : ");
                sb.append("#b").append(members.size()).append(" #n ").append("个远征队成员 : \n\r ");
                int i = 0, selection = 0;
                for (Entry<String, String> chr : members.entrySet()) {
                    i++;
