...
public class MapleItemInformationProvider {
    ...
    protected final MapleDataProvider stringData = MapleDataProviderFactory.getDataProvider(new File((System.getProperty("wzpath") != null ? System.getProperty("wzpath") : "") + "wz/String.wz"));
    //protected final MapleDataProvider equipData = MapleDataProviderFactory.getDataProvider("Character.wz");
    protected final MapleDataProvider equipData = MapleDataProviderFactory.getDataProvider(new File((System.getProperty("wzpath") != null ? System.getProperty("wzpath") : "") + "wz/Character.wz"));
    ...
    protected final Map<Integer, Integer> pachinko = new HashMap<>();
    ...
    private static final Logger logger = LogManager.getLogger(MapleItemInformationProvider.class);
 ...
 /*final MapleData lifesData = etcData.getData("ItemPotLifeInfo.img");
    ...
    /*public void runHairs() {
        File dataFile = new File((System.getProperty("path") != null ? System.getProperty("path") : "") + "wz/Character.wz/Hair");
        ...
        MapleData chrStringData = stringDataWZ.getData("Eqp.img").getChildByPath("Eqp/Hair");
 ...
 /*public void runFaces() {
        File dataFile = new File((System.getProperty("path") != null ? System.getProperty("path") : "") + "wz/Character.wz/Face");
        File strDataFile = new File((System.getProperty("path") != null ? System.getProperty("path") : "") + "wz/String.wz");
        MapleDataProvider chrData = MapleDataProviderFactory.getDataProvider(dataFile);
        MapleDataProvider stringDataWZ = MapleDataProviderFactory.getDataProvider(strDataFile);
        MapleData chrStringData = stringDataWZ.getData("Eqp.img").getChildByPath("Eqp/Face");
        for (MapleData c : chrStringData) {
            int chrid = Integer.parseInt(c.getName());
            String n = StringUtil.getLeftPaddedStr(chrid + ".img", '0', 12);
            try {
    ...
    public void runItems() {
        ...
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            ps = con.prepareStatement("SELECT * FROM wz_hairdata ORDER BY `hairid`");
            rs = ps.executeQuery();
            while (rs.next()) {
                hairfaceList.put(rs.getInt("hairid"), rs.getString("name"));
            }
            rs.close();
            ps.close();
            ps = con.prepareStatement("SELECT * FROM wz_facedata ORDER BY `faceid`");
            rs = ps.executeQuery();
            while (rs.next()) {
                hairfaceList.put(rs.getInt("faceid"), rs.getString("name"));
            }
            rs.close();
            ps.close();
 ...
 /*   public final String getName(final int itemId) {
    ...
    public static final ItemInformation getItemInformation(final int itemId) {
        if (itemId <= 0) {
            return null;
        }
    ...
    public MapleInventoryType getInventoryType2(final int itemId) {
        final byte type = (byte) (itemId / 1000000);
        if (type < 1 || type > 5) {
            return MapleInventoryType.UNDEFINED;
        }
    ...
    public boolean hairfaceExists(int hair) {
        return hairfaceList.containsKey(hair);
    }

    public final int getChairMountId(final int itemId) {
        if (chairMountId.containsKey(itemId)) {
            return chairMountId.get(itemId);
        }
        int ret = 0;
        final MapleData item = getItemData(itemId);
        if (item != null) {
    ...
    public final int getPachinko(final int itemId) {
        if (pachinko.containsKey(itemId)) {
            return pachinko.get(itemId);
        }
        int ret = 0;
        final MapleData item = getItemData(itemId);
        if (item != null) {
            final MapleData smEntry = item.getChildByPath("info/pachinko");
            if (smEntry == null) {
                ret = 0;
            } else {
                ret = (int) MapleDataTool.getInt(smEntry);
    ...
    public static void loadFaceHair() {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            try {
                ...
                FileoutputUtil.outError("log/资料库异常.txt", ex);
            }
            try {
