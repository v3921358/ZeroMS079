...
public class MapleTrade {
    ...
    public final void CompleteTrade() {
        if (this.exchangeItems != null) {
            ...
            if (ServerConfig.logs_trade) {
                FileoutputUtil.logToFile("logs/玩家操作/交易记录.txt", FileoutputUtil.NowTime() + " 账号: " + chr.get().getClient().getAccountName() + " 玩家: " + chr.get().getName() + " 和 " + partner.chr.get().getClient().getAccountName() + " " + partner.chr.get().getName() + " 交易获得 金币" + (exchangeMeso - GameConstants.getTaxAmount(exchangeMeso)) + " 和 " + exchangeItems.size() + "件物品[" + output + "]\r\n");
            }
        ...
        this.chr.get().getClient().getSession().write(CField.InteractionPacket.TradeMessage(this.tradingslot, (byte) 0x8));
    }
    ...
    public static final void completeTrade(MapleCharacter c) {
        ...
        if (partner.isLocked()) {
            ...
            if (local.getChr().getClient().getAccID() == partner.getChr().getClient().getAccID()) {
                local.getChr().ban("修改封包 - 同账号角色交易", true, true, false);
                partner.getChr().ban("修改封包 - 同账号角色交易", true, true, false);
                FileoutputUtil.logToFile("logs/Hack/ban/交易异常.txt", "时间: " + FileoutputUtil.NowTime() + " IP: " + local.getChr().getClient().getSessionIPAddress() + " MAC: " + local.getChr().getClient().getMacs() + " " + local.getChr().getName() + " 和 " + partner.getChr().getName() + " 为同个账号的角色进行交易。\r\n");
    ...
    public static final void startTrade(MapleCharacter c) {
        if (c.getTrade() == null) {
            ...
            c.getClient().getSession().write(CWvsContext.broadcastMsg(5, "不能同时做多件事情。"));
        }
    }

    public static final void startTradeCS(MapleCharacter c) {
        if (c.getTrade() == null) {
            c.setTrade(new MapleTrade((byte) 0, c));
    ...
    public static final void inviteTrade(MapleCharacter c1, MapleCharacter c2) {
        if (ServerConfig.Playertransaction) {
            c1.getClient().getSession().write(CWvsContext.broadcastMsg(1, "管理员从后台关闭了交易功能。"));
            return;
        }
        ...
        if ((c2 != null) && (c2.getTrade() == null)) {
            ...
            c1.getClient().getSession().write(CWvsContext.broadcastMsg(5, "对方正在和其他玩家进行交易中。"));
            cancelTrade(c1.getTrade(), c1.getClient(), c1);
        }
    }

    public static final void inviteTradeCS(MapleCharacter c1, MapleCharacter c2) {
        if ((c1 == null) || (c1.getTrade() == null)) {
            return;
        }
    ...
    public static final void visitTradeCS(MapleCharacter c1, MapleCharacter c2) {
        if ((c2 != null) && (c1.getTrade() != null) && (c1.getTrade().getPartner() == c2.getTrade()) && (c2.getTrade() != null) && (c2.getTrade().getPartner() == c1.getTrade())) {
            c1.getTrade().inTrade = true;
            c2.getClient().getSession().write(PlayerShopPacket.getTradePartnerAdd(c1, 1));
