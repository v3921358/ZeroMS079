...
public class Skill implements Comparator<Skill> {
    ...
    public static Skill loadFromData(final int id, final MapleData data, final MapleData delayData) {
        ...
        if (level != null) {
            ...
            Map<MapleStatEffect, Byte> map = new HashMap<>();
            for (final MapleData leve : data.getChildByPath("level")) {
                ...
                map.put(MapleStatEffect.loadSkillEffectFromData(leve, id, isBuff, Byte.parseByte(leve.getName()), null), Byte.parseByte(leve.getName()));
            }
            List<Map.Entry<MapleStatEffect, Byte>> list = new ArrayList<>(map.entrySet());
            Collections.sort(list, new Comparator<Map.Entry<MapleStatEffect, Byte>>() {
                @Override
                public int compare(Map.Entry<MapleStatEffect, Byte> o1, Map.Entry<MapleStatEffect, Byte> o2) {
                    return o1.getValue().compareTo(o2.getValue());
                }
            });
            for (Map.Entry<MapleStatEffect, Byte> mapping : list) {
                  ret.effects.add(mapping.getKey());
            }
