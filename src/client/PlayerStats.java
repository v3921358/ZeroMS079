...
public class PlayerStats implements Serializable {
    ...
    private void resetLocalStats(final int job) {
        ...
        add_matk = 0;
        crit_rate = 5;
        passive_sharpeye_min_percent = 0;
        passive_sharpeye_percent = 0;
        magic = 0;
        watk = 0;
        dodgeChance = 0;
        pvpDamage = 0;
    ...
    public void recalcLocalStats(boolean first_login, MapleCharacter chra) {
        ...
        for (Item item : chra.getInventory(MapleInventoryType.CASH).newList()) {
            ...
            } else if (item.getItemId() == 5650000) {//全屏宠吸
        ...
        int maxbasedamage = calculateMaxBaseDamage(watk, chra);

        int maxBaseMagicDamage = calculateMaxBaseMagicDamage();
        //int MaxBaseMagicDamage = calculateMaxBaseMagicDamage(magic);

        if (chra.isGM() || ServerConfig.logPackets) { //GM显示
            chra.dropMessage(5, "最大攻击：" + maxbasedamage);
            //chra.dropMessage(5, "最大攻击：" + maxbasedamage);
            chra.dropMessage(5, "魔法力：" + maxBaseMagicDamage);
        }
    ...
    private void handleBuffStats(MapleCharacter chra) {
        ...
        if (buff != null) {
            ...
            int critlevel = chra.getTotalSkillLevel(critSkill);
            if (critlevel > 0) {
                dam_r *= (size / 10) * (100 + critSkill.getEffect(critlevel).getDamage()) / 100.0;
            }
        }
        ...
        if (eff != null) {
            watk += eff.getWatk();
            dam_r *= (eff.getDamage() + 100.0D) / 100;
            accuracy += eff.getAcc();
        }
        ...
        switch (player.getJob()) { // Apply passive Critical bonus
            ...
            case 412: { // Night Lord
                ...
                if (critlevel > 0) {
                    ...
                    dam_r *= critSkill.getEffect(critlevel).getDamage() / 100.0;
                }
                break;
            }
            case 2412: { // Phantom
            ...
            case 1512: {
                ...
                if (critlevel > 0) {
                    this.crit_rate += (short) (critSkill.getEffect(critlevel).getProb());
                    //this.passive_sharpeye_min_percent += critSkill.getEffect(critlevel).getCriticalMin();
                    this.dam_r *= critSkill.getEffect(critlevel).getDamage() / 100.0;
                }
                break;
            }
            case 2111:
            case 2112: {
                ...
                critlevel = player.getTotalSkillLevel(critSkill);
                if (critlevel > 0) {
                    //dam_r *= (100 + critSkill.getEffect(critlevel).getDamage()) / 100.0;
    ...
    private void CalcPassive_Mastery(final MapleCharacter player) {
        ...
        switch (weaponType) {
            ...
            case AXE1H:
            case BLUNT1H:
                skil = GameConstants.isResistance(player.getJob()) ? 31100004 : (GameConstants.isKOC(player.getJob()) ? 11100000 : (player.getJob() > 112 ? 1200000 : 1100000)); //hero/pally
                break;
            case AXE2H:
            case SWORD1H:
            case SWORD2H:
    ...
    public int calculateMaxBaseDamage(int watk, MapleWeaponType2 weapon, MapleCharacter chra) {
        int mainstat, secondarystat;
        if (GameConstants.isA(chra.getJob(), 400) && weapon == MapleWeaponType2.DAGGER_OTHER) {
            weapon = MapleWeaponType2.DAGGER_THIEVES;
        }

        if (weapon == MapleWeaponType2.BOW || weapon == MapleWeaponType2.CROSSBOW || weapon == MapleWeaponType2.GUN) {
            mainstat = localdex;
            secondarystat = localstr;
        } else if (weapon == MapleWeaponType2.CLAW || weapon == MapleWeaponType2.DAGGER_THIEVES) {
            mainstat = localluk;
            secondarystat = localdex + localstr;
        } else {
            mainstat = localstr;
    ...
    public int calculateMaxBaseDamage(final int watk, MapleCharacter chra) {
        int maxbasedamage;
        final Item weapon_item = chra.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -11);
        if (weapon_item != null) {
            maxbasedamage = calculateMaxBaseDamage(watk, GameConstants.getWeaponType2(weapon_item.getItemId()), chra);
        } else if (GameConstants.isA(chra.getJob(), 500) || GameConstants.isA(chra.getJob(), 1500)) {
            double weapMulti = 3;
            if (chra.getId() % 100 != 0) {
                weapMulti = 4.2;
            }

            int attack = (int) Math.min(Math.floor((2 * chra.getLevel() + 31) / 3), 31);
    ...
    public int calculateMaxBaseMagicDamage() {
        ...
        return getTotalInt() + magic;
        //return  (int) Math.ceil(((4.4 * getTotalInt() + getTotalLuk()) / 100.0) * magic);
    }
    ...
    public final void connectData(final MaplePacketLittleEndianWriter mplew) {
        ...
        mplew.writeShort(hp);
        mplew.writeShort(maxhp);
        mplew.writeShort(mp);
        mplew.writeShort(maxmp);
    }
    ...
    public int calcMaxDamage(MapleStatEffect summonEffect, MapleCharacter player, boolean magic) {
        double maxDamage;

        if (magic) {
            int matk = Math.max(getTotalMagic(), 14);
            maxDamage = calculateMaxBaseMagicDamage(matk) * (0.05 * summonEffect.getMatk());
        } else {
            int watk = Math.max(getTotalWatk(), 14);
            ...
            if (weapon_item != null) {
                maxBaseDmg = calculateMaxBaseDamage(watk, GameConstants.getWeaponType2(weapon_item.getItemId()), player);
            } else {
                maxBaseDmg = calculateMaxBaseDamage(watk, MapleWeaponType2.SWORD1H, player);
            }
