...
public class MapleCharacter extends AnimatedMapleMapObject implements Serializable, MapleCharacterLook {
    ...
    private long lastCombo, lastfametime, keydown_skill, nextConsume, pqStartTime, lastDragonBloodTime,
            ...
            lastHPTime, lastMPTime, lastFamiliarEffectTime, lastExceedTime, lastDOTTime, exp, meso, lastExpirationTime, mobDirection = 0, lastresetTime = 0;
    ...
    private int accountid, id, hair, face, secondHair, secondFace, faceMarking, ears, tail, elf, mapid, fame, pvpExp, pvpPoints, totalWins, totalLosses,
            ...
            remainingAp, honourExp, honorLevel, runningLight, runningLightSlot, runningDark, runningDarkSlot, luminousState, csType = -1, bookCover, loginkey, MobVac_X = 0, MobVac = 0, FastMove = 0,OfflineTime = 0, beans, beansNum, beansRange;
    ...
    private final Map<Skill, SkillEntry> linkskills;
    ...
    private long todayOnlineTime;//在线时间
    private long totalOnlineTime;//在线时间
    private long monthlyCard;//在线时间
    ...
    private int lpjf = 0;
    private int cz = 0;
    private int 骑宠ID = 0,伤害值 = 0,累积击杀;
    private boolean testingdpm = false;
    ...
    public static MapleCharacter getDefault(final MapleClient client, final JobType type) {
        ...
        ret.map = null;
        ret.exp = 0;
        ret.gmLevel = (byte) (ServerConstants.isBetaForAdmins ? 4 : 0);
        client.setGm(ServerConstants.isBetaForAdmins ? true : false);
        ...
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    ret.loginkey = rs.getInt("loginkey");
                }
            }
            ps.close();
    ...
    public static MapleCharacter ReconstructChr(final CharacterTransfer ct, final MapleClient client, final boolean isChannel) {
        ...
        ret.monthlyCard = ct.monthlyCard;
        ret.lpdj = ct.lpdj;
        ret.lpjf = ct.lpjf;
        ret.cz = ct.cz;
        ...
        ret.exp = ret.level >= (GameConstants.isKOC(ret.job) ? ret.kocmaxlevel : ret.maxLevel) ? 0 : ct.exp;//骑士团 冒险家职业
        ...
        for (final Map.Entry<Integer, SkillEntry> qs : ct.Skills.entrySet()) {
            ...
            if (SkillFactory.getSkill(qs.getKey()).isLinkSkills()) {
                ret.linkskills.put(SkillFactory.getSkill(qs.getKey()), qs.getValue());
            }
        }
    ...
    public static MapleCharacter loadCharFromDB(int charid, MapleClient client, boolean channelserver, final Map<Integer, CardData> cads) {
        ...
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            ret.exp = ret.level >= (GameConstants.isKOC(ret.job) ? ret.kocmaxlevel : ret.maxLevel) ? 0 : rs.getLong("exp");
            ...
            ret.todayOnlineTime = rs.getLong("todayOnlineTime");
            ret.totalOnlineTime = rs.getLong("totalOnlineTime");
            ret.monthlyCard = rs.getLong("monthlyCard");
            ret.lpdj = rs.getInt("lpdj");
            ret.lpjf = rs.getInt("lpjf");
            ret.cz = rs.getInt("cz");
            ...
            ret.reborns = rs.getInt("reborns");//转生
            ret.apstorage = rs.getInt("apstorage");
            ret.骑宠ID = rs.getInt("qcID");
            ret.伤害值 = rs.getInt("hurt");
            ret.累积击杀 = rs.getInt("累积击杀");
 ...
 /*if (rs.next()) {
                ...
                while (rs.next()) {
                    ...
                    if (skil != null && GameConstants.isApplicableSkill(skid)) {
                        ...
                        if ((position >= 0) && (position < 13) && (phantom < 13)) {
                            if ((GameConstants.isPhantom(ret.job)) && (skil.getSkillByJobBook() != -1)) {
                            ...
                            if (skil.isLinkSkills()) {
                                ret.linkskills.put(skil, new SkillEntry(skl, msl, rs.getLong("expiration"), teachId));
                            }
            ...
            FileoutputUtil.outputFileError(FileoutputUtil.MysqlEx_Log, ess);
            ...
            try {
                ...
                FileoutputUtil.outputFileError(FileoutputUtil.MysqlEx_Log, ignore);
    ...
    public static int getQuestKillCount(MapleCharacter chr, final int mobid) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            PreparedStatement pse = con.prepareStatement("SELECT count FROM queststatusmobs WHERE queststatusid = ?");
    ...
    public static void saveNewCharToDB(final MapleCharacter chr, final JobType type, short db) {
        ...
        try {
            ...
            ps = con.prepareStatement("INSERT INTO characters (level, str, dex, luk, `int`, hp, mp, maxhp, maxmp, sp, hsp, ap, skincolor, gender, job, hair, face, faceMarking, ears, tail, map, meso, party, buddyCapacity, pets, subcategory, elf, friendshippoints, chatcolour, gm, accountid, name, qcID,hurt ,累积击杀,world,OfflineTime)"
                    + "                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", DBConPool.RETURN_GENERATED_KEYS);
            int index = 0;
...
//            }
          ...
          //  ps.setInt(++index, ServerConfig.BEGINNER_SPAWN_MAP); //设置出生地图
            ...
            ps.setInt(++index, chr.骑宠ID);
            ps.setInt(++index, chr.伤害值);
            ps.setInt(++index, chr.累积击杀);
            ps.setByte(++index, chr.world);
            ps.setInt(++index, chr.OfflineTime);
            ps.executeUpdate();
            ...
            ps.setInt(1, chr.id);
            ps.setByte(2, (byte) ServerConfig.defaultInventorySlot); // 初始裝備欄Eq
            ps.setByte(3, (byte) ServerConfig.defaultInventorySlot); // 初始消耗欄Use
            ps.setByte(4, (byte) ServerConfig.defaultInventorySlot); // 初始裝飾欄Setup
            ps.setByte(5, (byte) ServerConfig.defaultInventorySlot); // 初始其他欄ETC
    ...
    public void saveToDB(boolean dc, boolean fromcs) {
        ...
        try {
            ...
            ps = con.prepareStatement("UPDATE characters SET level = ?, fame = ?, str = ?, dex = ?, luk = ?, `int` = ?, exp = ?, hp = ?, mp = ?, maxhp = ?, maxmp = ?, sp = ?, hsp = ?, ap = ?, gm = ?, skincolor = ?, gender = ?, job = ?, hair = ?, face = ?, faceMarking = ?, ears = ?, tail = ?, map = ?, meso = ?, hpApUsed = ?, spawnpoint = ?, party = ?, monsterbookcover = ?, buddyCapacity = ?, pets = ?, subcategory = ?, marriageId = ?, currentrep = ?, totalrep = ?, gachexp = ?, fatigue = ?, charm = ?, charisma = ?, craft = ?, insight = ?, sense = ?, will = ?, totalwins = ?, totallosses = ?, pvpExp = ?, pvpPoints = ?, reborns = ?, apstorage = ?, elf = ?, honourExp = ?, honourLevel = ?, friendshippoints = ?, friendshiptoadd = ?, chatcolour = ?, todayOnlineTime = ?, totalOnlineTime = ?, monthlyCard = ?, lpdj = ?, lpjf = ?, cz = ?, name = ?, qcID = ?, hurt = ?, 累积击杀 = ? WHERE id = ?", DBConPool.RETURN_GENERATED_KEYS);
            int index = 0;
            ...
            ps.setLong(++index, level >= (GameConstants.isKOC(job) ? kocmaxlevel : maxLevel) ? 0 : exp);
            ...
            ps.setLong(++index, todayOnlineTime);
            ps.setLong(++index, totalOnlineTime);
            ps.setLong(++index, monthlyCard);
            ps.setInt(++index, lpdj);
            ps.setInt(++index, lpjf);
            ps.setInt(++index, cz);
            ps.setString(++index, name);
            ps.setInt(++index, 骑宠ID);
            ps.setInt(++index, 伤害值);
            ps.setInt(++index, 累积击杀);
            ps.setInt(++index, id);
    ...
    public final void updateQuest(final MapleQuestStatus quest, final boolean update) {
        ...
        if (!(quest.isCustom())) {
            ...
            if (quest.getStatus() == 1 && !update) {
                if (quest.getQuest().getId() != 6029 && quest.getCustomData() != "111") {
                    client.getSession().write(CField.updateQuestInfo(this, quest.getQuest().getId(), quest.getNpc(), (byte) 8));//was10
                }
            }
        }
    }
    ...
    public void doDragonBlood() {
        ...
        if (stats.getHp() - bloodEffect.getX() <= 1) {
            ...
            client.getSession().write(EffectPacket.showOwnBuffEffect(bloodEffect.getSourceId(), 5, getLevel(), bloodEffect.getLevel()));
            map.broadcastMessage(MapleCharacter.this, EffectPacket.showBuffeffect(getId(), bloodEffect.getSourceId(), 5, getLevel(), bloodEffect.getLevel()), false);
        }
    }
    ...
    public void doFish(long now) {
        ...
        while (!passed) {
            int randval = FishingRewardFactory.getInstance().getNextRewardType();
            switch (randval) {
                case 0: // Meso
                    final int money = ServerConfig.Fishinggoldcoinsseveral * Randomizer.rand(expMulti ? 15 : 10, expMulti ? 15000 : 10000);

                    gainMeso(money/2, true);
                    passed = true;
                    break;
                case 1: // EXP
                    final long experi = ServerConfig.Fishingexperienceratio * Randomizer.nextInt((int) Math.min((Math.abs(getNeededExp() / 200) + 1), 500000));
                    gainExp(expMulti ? experi * 3 / 4 : experi, true, false, true);
                    passed = true;
                    break;
                default:
                    final FishingReward item = FishingRewardFactory.getInstance().getNextRewardItemId();
                    if (MapleItemInformationProvider.getInstance().itemExists(item.getItemId())) {
                        MapleInventoryManipulator.addById(client, item.getItemId(), (short) 1, "钓鱼" + " 时间 " + FileoutputUtil.CurrentReadable_Date());
                        passed = true;
                    }
                    break;
            }
        ...
        if (passed) {
            client.getSession().write(EffectPacket.showForeignEffect(19));
        }
...
//the sp isnt fixed oh hold on
   ...
   {
     MapleMap target = this.client.getChannelServer().getMapFactory().getMap(mapid);
    ...
    private void changeMapInternal(final MapleMap to, final Point pos, byte[] warpPacket, final MaplePortal pto) {
        ...
        if (getChair() > 0) {
            cancelFishingTask();
            setChair(0);
            getClient().getSession().write(CField.cancelChair(-1));
        ...
        if (getTrade() != null) {
            ...
            if (getTrade().getPartner() != null) {
                ...
                if (local.isLocked() && partners.isLocked()) {
                    ...
                    MapleTrade.cancelTrade(getTrade(), getClient(), getClient().getPlayer());
                }
            } else {
                MapleTrade.cancelTrade(getTrade(), client, client.getPlayer());
            }
        }
...
//            client.getSession().write(EffectPacket.updateDeathCount(99)); //for fun
    ...
    public void cancelChallenge() {
        if (challenge != 0 && client.getChannelServer() != null) {
            ...
            if (chr != null) {
                chr.dropMessage(6, getName() + " 拒绝了你的要求.");
                chr.setChallenge(0);
            }
            dropMessage(6, "拒绝挑战.");
            challenge = 0;
    ...
    public void changeJob(short newJob) {
        try {
            ...
            /*if (!GameConstants.isBeginnerJob(newJob) && !GameConstants.isDualBladeNoSP(newJob) && !GameConstants.isZero(newJob) && !GameConstants.isBeastTamer(newJob)) {
                addJobSkills();
                if (GameConstants.isSeparatedSp(newJob)) {
                    ...
                    int changeSp = (newJob == 2200 || newJob == 2210 || newJob == 2211 || newJob == 2213 ? 3 : 1);
                    if (GameConstants.isResistance(job) && newJob != 3100 && newJob != 3200 && newJob != 3300 && newJob != 3500 && newJob != 3600) {
                        changeSp = 3;
                    }
                ...
                updateSingleStat(MapleStat.AVAILABLESP, getRemainingSp()); // we don't care the value here
            }*/
             ...
             if (newJob != 0 && newJob != 1000 && newJob != 2000 && newJob != 2001 && newJob != 3000) {
                if (isEv) {
                    remainingSp[GameConstants.getSkillBook(newJob,0)] += 3;
                     //updateSingleStat(MapleStat.AVAILABLESP, remainingAp);
           ...
           // client.sendPacket(MaplePacketCreator.updateSp(this, false));
            ...
            map.broadcastMessage(this, EffectPacket.showForeignEffect(getId(), 9), false);
            silentPartyUpdate();
            guildUpdate();
            familyUpdate();
            if (dragon != null) {
...
//        System.out.println(start + " | " + end);
    ...
    public void gainSP(int sp) {
        ...
        updateSingleStat(MapleStat.AVAILABLESP, remainingAp); // we don't care the value here
        ...
        try {
            ...
            if (!GameConstants.isBeginnerJob(job) && !inPVP() /*&& !GameConstants.isAzwanMap(getMapId())*/) {
                ...
                if (charms > 0) {
                    ...
                    client.getSession().write(EffectPacket.useCharm((byte) charms, (byte) 0, true));//7
                } else {
                    float diepercentage;
            ...
            FileoutputUtil.outputFileError("logs/死亡出错.txt", ex);
            System.err.println(ex);
        }
        getClient().removeClickedNPC();
        NPCScriptManager.getInstance().dispose(getClient());
    ...
    public void gainExp(final long total, final boolean show, final boolean inChat, final boolean white) {
        ...
        try {
            ...
            if (this.level < GameConstants.等级上限) {

                if (level >= (GameConstants.isKOC(job) ? kocmaxlevel : maxLevel)) {
                    setExp(0);
                    //if (exp + total > needed) {
                    //    setExp(needed);
                    //} else {
                    ...
                    if (tot >= needed) {
                        ...
                        leveled = true;
                        if (level >= (GameConstants.isKOC(job) ? kocmaxlevel : maxLevel) && !isIntern()) {
                            setExp(0);
                        } else {
                            needed = getNeededExp();
                            if (exp >= needed) {
    ...
    public void gainExpMonster(int gain, final boolean show, final boolean white, final byte pty, int Class_Bonus_EXP, int Equipment_Bonus_EXP, int Premium_Bonus_EXP, boolean partyBonusMob, final int partyBonusRate) {
        if (!isAlive()) {
            return;
        }
        expirationTask(false, false);
        //expirationTask(true, false);
        boolean merch = World.hasMerchant(getAccountID(), getId());
        if (merch) {
            int 雇佣经验 = (int) Math.ceil(gain * ServerConfig.Employmentexperience);
            gain = gain + 雇佣经验;
        }
        int Marriage_EXP = 0;
        if (getMarriageId() > 0) {
            MapleCharacter victim = ChannelServer.getInstance(getClient().getChannel()).getPlayerStorage().getCharacterById(getMarriageId());
            if (victim != null) {
                if (victim.getMapId() == getMapId()) {
                    Marriage_EXP = (int) Math.ceil(gain * ServerConfig.Marriageexperience);
                }
            }
        }

        long total = gain + Class_Bonus_EXP + Equipment_Bonus_EXP + Premium_Bonus_EXP + Marriage_EXP;
        int partyinc = 0;
        long prevexp = getExp();
        if (pty > 1) {
        ...
        if (this.level < GameConstants.等级上限) {
            ...
            if (gain != 0) {
                ...
                updateSingleStat(MapleStat.EXP, getExp());
                if (show) { // still show the expgain even if it's not there
                    client.getSession().write(InfoPacket.GainEXP_Monster(gain, white, partyinc, Class_Bonus_EXP, Equipment_Bonus_EXP, Premium_Bonus_EXP, Marriage_EXP));
                }
            }
        }
    }
...
//            client.getSession().write(InventoryPacket.updateSpecialItemUse(item, type == MapleInventoryType.EQUIPPED ? (byte) 1 : type.getType(), this));
    ...
    public final void expirationTask(boolean pending, boolean firstLoad) {
        ...
        if (pending) {
            ...
            if (pendingSkills != null) {
                ...
                for (Skill z : pendingSkills.keySet()) {
                    client.getSession().write(CWvsContext.broadcastMsg(5, "[" + SkillFactory.getSkillName(z.getId()) + "] 技能时间已过期。"));
                }
            } //not real msg
            pendingSkills = null;
            return;
        ...
        for (final Triple<MapleInventoryType, Item, Boolean> itemz : toberemove) {
            item = itemz.getMid();
            if (item == null) {
                FileoutputUtil.logToFile("logs/data/道具到期.txt", new StringBuilder().append(getName()).append(" 检测道具已经过期，但道具为空，无法继续执行。").toString(), true);
                continue;
            }
            ...
            if (firstLoad) {
                ...
                MapleInventoryManipulator.removeFromSlot(client, (MapleInventoryType) itemz.getLeft(), item.getPosition(), item.getQuantity(), false);
            }
            if (itemz.getRight() && getInventory(GameConstants.getInventoryType(item.getItemId())).getNextFreeSlot() > -1) {
                ...
                //getClient().getSession().write(InventoryPacket.clearInventoryItem(itemz.getLeft(), item.getPosition(), false));
        ...
        for (final Item itemz : tobeunlock) {
            ...
            forceReAddItem_Flag(itemz, GameConstants.getInventoryType(itemz.getItemId()));
            dropMessage(5, new StringBuilder().append("封印道具[").append(ii.getName(itemz.getItemId())).append("]封印时间已过期。").toString());
    ...
    public void gainMeso(long gain, boolean show, boolean inChat, boolean enableActions) {
    ...
    public void controlMonster(MapleMonster monster, boolean aggro) {
        ...
        client.getSession().write(MobPacket.controlMonster(monster, false, aggro, false));
    ...
    public Map<Skill, SkillEntry> getLinkSkills() {
    ...
    public final int maxLevel = ServerConfig.maxlevel;
    public final int kocmaxlevel = ServerConfig.kocmaxlevel;

    public void levelUp() {
        if (this.level >= GameConstants.等级上限) {
            dropMessage(5, "【世界封印】 受到封印制约,您将无法再获取经验.请小伙伴们一起突破冒险岛世界等级把!");
            return;
        }
        if (GameConstants.isKOC(job)) {
            if (getLevel() >= kocmaxlevel) {
                return;
            }
        ...
        if (GameConstants.isBeginnerJob(job)) { // 新手
            maxhp += Randomizer.rand(12, 16);
            maxmp += Randomizer.rand(10, 12);
        } else if (job >= 100 && job <= 132) { // 冒险家战士职业群
        ...
        } else if (job >= 1100 && job <= 1112) { // 骑士团战士职业群
            ...
            maxhp += Randomizer.rand(24, 28);
            maxmp += Randomizer.rand(4, 6);
        } else if (job >= 200 && job <= 232) { // 冒险家法师职业群
        ...
        } else if (job >= 1200 && job <= 1211) { // 骑士团法师职业群
        ...
        if (level <= 10 && GameConstants.isBeginnerJob(job)) {
        ...
        if (level >= 8 && !GameConstants.isBeginnerJob(job)) {
            remainingSp[GameConstants.getSkillBook(job, 0)] += 3;
        }
      ...
      /*  if (!isIntern()) {
            ...
            sb.append(getName()).append("在地图:" + map.getMapName()).append(",终于达到了").append(level).append("级,大家一起祝贺下吧。");
            World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(6, sb.toString()));
        }*/
        checkCustomReward(level);
    }
        public void DoLevelMsg() {
        if (ServerConfig.Upgradeconsultation) {
            if (!isGM()) {
                if (level >= 1) {
                    String 最高等级玩家 = NPCConversationManager.获取最高等级玩家名字();
                    final StringBuilder sb = new StringBuilder("[升级快讯]: 恭喜 玩家");
                    final Item medal = getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -26);
                    if (guildid > 0) {
        ...
        if (GameConstants.isAran(job)) {
            switch (level) {
                case 30:
                    getClient().getSession().write(CField.startMapEffect("恭喜达到30级请回金银岛二转吧。", 5120000, true));
                    break;
                case 70:
                    getClient().getSession().write(CField.startMapEffect("恭喜达到70级请到冰原雪域长老公馆三转吧。", 5120000, true));
                    break;
                case 120:
                    getClient().getSession().write(CField.startMapEffect("恭喜达到120级请回神木村祭司森林四转吧。", 5120000, true));
                    break;
            }
        }
        if (GameConstants.isKOC(job) && level == 70) {
            getClient().getSession().write(CField.startMapEffect("恭喜达到70等请到冰封雪域三转吧。", 5120000, true));
        }
        //双刀自动转职
       /* if (getSubcategory() == 1) {
            switch (level) {
    ...
    public void sendMacros() {
        client.getSession().write(CField.getMacros0());
        for (int i = 0; i < 5; i++) {
    ...
    public void setMaxHp(int amount) {
        ...
        updateSingleStat(MapleStat.MAXHP, stats.getMaxHp());
    }

    public void setMaxMp(int amount) {
        getStat().setMaxMp(amount, this);
        updateSingleStat(MapleStat.MAXMP, stats.getMaxMp());
    }

    public void addMaxHp(int amount) {
        getStat().setMaxHp(amount + getMaxHp(), this);
        updateSingleStat(MapleStat.MAXHP, stats.getMaxHp());
    }

    public void addMaxMp(int amount) {
        getStat().setMaxMp(amount + getMaxMp(), this);
        updateSingleStat(MapleStat.MAXMP, stats.getMaxMp());
    }
    ...
    public void modifyCSPoints(int type, int quantity, boolean show) {
        switch (type) {
            case 4:
                ...
                //}
                setNxcredit(nx + quantity);
                break;
            case 1:
                int ac = getACash();
                if (ac + quantity < 0) {
                    if (show) {
                        dropMessage(-1, "你没有足够的赞助点数。");
                    }
                    return;
                }
                //if (quantity > 0) {
                //    quantity = (quantity / 2); //stuff is cheaper lol
                //}
                setACash(ac + quantity);
                break;
            case 2:
                int mp = getMaplePoints();
                if (mp + quantity < 0) {
                    if (show) {
                ...
                setMaplePoints(mp + quantity);
                client.getSession().write(CWvsContext.updateMaplePoint(this));
                break;
            default:
                break;
        }
        if (show && quantity != 0) {
            dropMessage(-1, "你 " + (quantity > 0 ? "获得了 " : "消耗了 ") + quantity + (type == 1 ? " 点券。" : type == 2 ? " 抵用券。" : type == 4 ? " 赞助的点数。" : " 未知。"));
            client.getSession().write(EffectPacket.showForeignEffect(19));
        }
    }
    ...
    public void sendNote(String to, String msg, int fame) {//邮件
    ...
    public int[] getRocks() {
        ...
        int max;
        int temp;
        for (out = rocks.length - 1; out > 0; out--) {
            max = out;
            for (in = out - 1; in > 0; in--) {
                if (rocks[max] < rocks[in]) {
                    max = in;
                }
            }
            if (out != max) {
    ...
    public int[] getRegRocks() {
        ...
        int max;
        int temp;
        for (out = regrocks.length - 1; out > 0; out--) {
            max = out;
            for (in = out - 1; in > 0; in--) {
                if (regrocks[max] < regrocks[in]) {
                    max = in;
                }
            }
            if (out != max) {
                temp = regrocks[out];
                regrocks[out] = regrocks[max];
                regrocks[max] = temp;
            }
        }
        return regrocks;
    ...
    public MapleCharacter cloneLooks() {
        MapleClient cloneclient = new MapleClient(null, null, new MapleSession(new MockIOSession()));
        ...
        ret.totalOnlineTime = totalOnlineTime;
        ret.monthlyCard = monthlyCard;
        ret.lpdj = lpdj;
        ret.lpjf = lpjf;
        ret.cz = cz;
    ...
    public final void cloneLook() {//镜像克隆分身术
        if (clone || inPVP()) {
            return;
        }
        for (int i = 0; i < clones.length; i++) {
    ...
    public final int getCloneSize() {//镜像克隆分身术
        int z = 0;
        for (int i = 0; i < clones.length; i++) {
            if (clones[i].get() != null) {
                z++;
    ...
    public void spawnClones() {//镜像克隆分身术
    ...
    public void changeChannel(final int channel) {
     ...
     //   if (双爆频道开关 == 0) { //双爆频道票 5220002
            //if (channel == Integer.parseInt(ServerProperties.getProperty("world.双爆频道"))) {
            if (channel == Channelnumber) {
                if (!haveItem(Doubleexplosionchannel, 1, false, false)) {
                    dropMessage(1, "对不起，你缺少特殊物品，无法进入 " + Channelnumber + " 频道, 该频道为双倍爆率频道！");
                    client.getSession().write(CWvsContext.enableActions());
                    return;
                }  
            }
        ...
        String ipAddr = ChannelServer.getInstance(getClient().getChannel()).getIP().split(":")[0];
        client.getSession().write(CField.getChannelChange(client, ipAddr, Integer.parseInt(toch.getIP().split(":")[1])));
    ...
    public void forceChangeChannel(final int channel) {
        ...
        saveToDB(false, false);
        getMap().removePlayer(this);
        if (!LoginServer.CanLoginKey(this.getLoginKey(), this.getAccountID())) {
            FileoutputUtil.logToFile("logs/Data/客户端key异常.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + client.getSession().getRemoteAddress().toString().split(":")[0] + " 账号: " + client.getAccountName() + " 客户端key：" + LoginServer.getLoginKey(this.getAccountID()) + " 服务端key：" + this.getLoginKey() + " 更换频道");
            World.Broadcast.broadcastGMMessage(CWvsContext.broadcastMsg(6, "[系统警告] 非法更换频道 账号 " + client.getAccountName()));
            client.getSession().close();
            return;
        }
    ...
    public void changeRemoval(boolean dc) {
        try {
            ...
            System.err.println("Error changeRemoval : " + e);
            FileoutputUtil.log(FileoutputUtil.ChangeRemovalEx_Log, "Error changeRemoval : " + e);
        }
    }
    ...
    public void setAndroid(MapleAndroid a) {
        ...
        if ((this.map != null) && (a != null)) {
            ...
            this.map.broadcastMessage(CField.showAndroidEmotion(getId(),
                    (byte) Randomizer.nextInt(5), (byte) Randomizer.nextInt(1)));
        }
    }
                    ...
                    if (divineShield.makeChanceResult()) {
                        if (attacke instanceof MapleMonster) {
                            ...
                            for (final MapleMapObject mo : affected) {
                                ...
                                final int theDmg = (int) (divineShield.getDamage() * getStat().calculateMaxBaseDamage(getStat().getTotalWatk(), this) / 100.0);
    ...
    public int getPQLog(String pqid) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            int ret_count = 0;
            PreparedStatement ps;
            ps = con.prepareStatement("SELECT COUNT(*) FROM pqlog WHERE charid = ? and pqid = ? and lastattempt >= subtime(current_timestamp, '1 0:0:0.0')");
            ps.setInt(1, id);
            ps.setString(2, pqid);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
...
//    }
    ...
    public void setPQLog(String pqid) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
            ps = con.prepareStatement("INSERT INTO pqlog (accid, charid, pqid) values (?,?,?)");
            ps.setInt(1, accountid);
            ps.setInt(2, id);
            ps.setString(3, pqid);
            ps.executeUpdate();
    ...
    public void toggleCustomBGState() {
        ...
        for (byte i = 0; i < 127; i++) {
            client.getSession().write(CField.removeBGLayer((getCustomBGState() == 1), 0, i, 0));
            //duration 0 = forever map 0 = current map
        }
    }
...
//        client.getSession().write(EffectPacket.updateDeathCount(deathCount));
    ...
    public void updateSkills() {
        ...
        client.getSession().write(CField.getKeymap(getKeyLayout()));
        client.getPlayer().changeSkillsLevel(sa);
    }

    public final void maxSkills() {//满技能
        for (Skill skil : SkillFactory.getAllSkills()) {
   ...
   public final void clearSkills()//消技能
   {
        for (Skill skil : SkillFactory.getAllSkills()) {
   ...
   public final void openSkill(MapleCharacter victim) {
        for (Skill skil : SkillFactory.getAllSkills()) {
    ...
    public boolean canExpiration(long now) {   //检测角色道具是否到期的处理
    ...
    public int getNxcredit(int aid) {
        int nx = 0;
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            int cid = aid;
            ResultSet rs;
    ...
    public void setNxcredit(int nx) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
            ps = con.prepareStatement("UPDATE accounts SET nxCredit = ? Where id = ?");
            ps.setInt(1, nx);
    ...
    public void setNxcredit(int aid, int nx) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
            ps = con.prepareStatement("UPDATE accounts SET nxCredit = ? Where id = ?");
            ps.setInt(1, nx);
    ...
    public int getMaplePoints(int aid) {
        int nx = 0;
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            int cid = aid;
            ResultSet rs;
    ...
    public void setMaplePoints(int nx) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
    ...
    public void setMaplePoints(int aid, int nx) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
            ps = con.prepareStatement("UPDATE accounts SET mPoints = ? Where id = ?");
            ps.setInt(1, nx);
    ...
    public void setBossDayLog(String bossid) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
    ...
    public int getBossDayLog(String bossid) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            int ret_count;
            PreparedStatement ps;
            ps = con.prepareStatement("select count(*) from bossdaylog where characterid = ? and bossid = ?");
            ps.setInt(1, id);
            ps.setString(2, bossid);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
    ...
    public void isSquadPlayerID() {
        ...
        if (isSquadPlayerID.size() > 0) {
            for (final Entry<String, String> emeim : isSquadPlayerID.entrySet()) {
                String em = emeim.getKey();
                String eim = emeim.getValue();
                final EventManager bossem = getClient().getChannelServer().getEventSM().getEventManager(em);
                if (bossem != null && bossem.getInstance(eim) != null) {
                    EventInstanceManager bosseim = bossem.getInstance(eim);
                    if (bosseim != null && bosseim.getProperty("isSquadPlayerID_" + getId()) != null) {
                        String propsa = bosseim.getProperty("isSquadPlayerID_" + getId());
                        if ((propsa != null) && propsa.equals("0")) {
                            bosseim.setProperty("isSquadPlayerID_" + getId(), "1");
                        }
                    }
                }
            }
    ...
    public void dropNPC(String message) {
        client.getSession().write(NPCPacket.getNPCTalk(9010000, (byte) 0, message, "00 00", (byte) 0));
    }

    public void dropNPC(int npc, String message) {
        client.getSession().write(NPCPacket.getNPCTalk(npc, (byte) 0, message, "00 00", (byte) 0));
    }
    ...
    public void gainTodayOnlineTime(long s) {
        todayOnlineTime += s;
    }

    public void setTodayOnlineTime(long s) {
        todayOnlineTime = s;
    }
    ...
    public void gainTotalOnlineTime(long s) {
        totalOnlineTime += s;
    }

    public void setTotalOnlineTime(long s) {
        totalOnlineTime = s;
    }

    public long getMonthlyCard() {
        return monthlyCard;
    }

    public void gainMonthlyCard(long s) {
        monthlyCard += s;
    }

    public void setMonthlyCard(long s) {
        monthlyCard = s;
    }
    ...
    public void setACash(int aid, int nx) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
            ps = con.prepareStatement("UPDATE accounts SET ACash = ? Where id = ?");
            ps.setInt(1, nx);
    ...
    public int getTheAgentCount(int bossid) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            int ret_count = 0;
            PreparedStatement ps;
            ps = con.prepareStatement("select count(*) from theagent where chrid2 = ?");
            ps.setInt(1, bossid);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                ret_count = rs.getInt(1);
    ...
    public int getTheAgent() {
        int nx = 0;
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            int cid = getId();
            ResultSet rs;
            try (PreparedStatement limitCheck = con.prepareStatement("SELECT * FROM theagent WHERE chrid=" + cid + "")) {
                rs = limitCheck.executeQuery();
                if (rs.next()) {
                    nx = rs.getInt("chrid2");
                }
    ...
    public void 设置累积击杀(int 参_累积击杀) {
        this.累积击杀 = 参_累积击杀;
    }
    
    public void 角色伤害更新(int 伤害) {
        设置伤害值(伤害);
        try (Connection con1 = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
            ps = con1.prepareStatement("UPDATE characters SET hurt=? WHERE id=?");
            ps.setInt(1, 伤害);
            ps.setInt(2, getId());
            ps.executeUpdate();
            ps.close();
    ...
    public int getTheAgent(int aid) {
        int nx = 0;
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            int cid = aid;
            ResultSet rs;
    ...
    public void setTheAgent(int nx) {

        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
            ps = con.prepareStatement("insert into theagent (chrid, chrid2) values (?,?)");
            ps.setInt(1, getId());
            ps.setInt(2, nx);
            ps.executeUpdate();
            ps.close();
    ...
    public void setTheAgent(int aid, int nx) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
            ps = con.prepareStatement("insert into theagent (chrid, chrid2) values (?,?)");
            ps.setInt(1, aid);
    ...
    public MapleCharacter getCharacterById2(int cid) {
        MapleCharacter chr = getOnlineCharacterById(cid);
        if (chr != null) {
            return chr;
        }
    ...
    public int getCharacterIdByName2(String name) {
        int id = -1;
        MapleCharacter chr = getOnlineCharacterByName2(name);
        if (chr != null) {
            return chr.getId();
        }
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
    ...
    public MapleCharacter getOnlineCharacterByName2(String name) {
        MapleCharacter chr = null;
        if (World.Find.findChannel(name) >= 1) {
            chr = ChannelServer.getInstance(World.Find.findChannel(name)).getPlayerStorage().getCharacterByName(name);
            if (chr != null) {
    ...
    public int getAcDayLog(String bossid) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            int ret_count = 0;
            PreparedStatement ps;
            ps = con.prepareStatement("select count(*) from acdaylog where accid = ? and bossid = ?");
    ...
    public void setAcDayLog(String bossid) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
            ps = con.prepareStatement("insert into acdaylog (accid, bossid) values (?,?)");
            ps.setInt(1, getClient().getAccID());
            ps.setString(2, bossid);
            ps.executeUpdate();
            ps.close();
    ...
    public int getAcForEverLog(String bossid) {
        int nx = -1;
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            int aid = getClient().getAccID();
            ResultSet rs;
            try (PreparedStatement limitCheck = con.prepareStatement("SELECT * FROM acforeverlog WHERE accid = ? and bossid = ?")) {
                limitCheck.setInt(1, aid);
                limitCheck.setString(2, bossid);
                rs = limitCheck.executeQuery();
                if (rs.next()) {
                    nx = rs.getInt("size");
                }
    ...
    public int getAcForEverLogCount(String bossid) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            int ret_count = -1;
            PreparedStatement ps;
            ps = con.prepareStatement("select count(*) from acforeverlog where accid = ? and bossid = ?");
            ps.setInt(1, getClient().getAccID());
            ps.setString(2, bossid);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public void setAcForEverLog(String bossid) {
        if (getAcForEverLog(bossid) <= -1) {
            try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
                PreparedStatement ps;
                ps = con.prepareStatement("insert into acforeverlog (accid, bossid) values (?,?)");
                ps.setInt(1, getClient().getAccID());
                ps.setString(2, bossid);
                ps.executeUpdate();
                ps.close();
        ...
        int size = getAcForEverLog(bossid);
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
            ps = con.prepareStatement("UPDATE acforeverlog SET size = ? Where accid = ? and bossid = ?");
            ps.setInt(1, size + 1);
            ps.setInt(2, getClient().getAccID());
    ...
    public int getBossForEverLog(String bossid) {
        int nx = -1;
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            int cid = getId();
            ResultSet rs;
            try (PreparedStatement limitCheck = con.prepareStatement("SELECT * FROM bossforeverlog WHERE characterid= ? and bossid = ?")) {
                limitCheck.setInt(1, cid);
                limitCheck.setString(2, bossid);
                rs = limitCheck.executeQuery();
                if (rs.next()) {
                    nx = rs.getInt("size");
    ...
    public int getBossForEverLogCount(String bossid) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            int ret_count = -1;
            PreparedStatement ps;
            ps = con.prepareStatement("select count(*) from bossforeverlog where characterid = ? and bossid = ?");
            ps.setInt(1, getId());
            ps.setString(2, bossid);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public void setBossForEverLog(String bossid) {

        if (getBossForEverLog(bossid) <= -1) {
            try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
                PreparedStatement ps;
                ps = con.prepareStatement("insert into bossforeverlog (characterid, bossid) values (?,?)");
                ps.setInt(1, getId());
                ps.setString(2, bossid);
                ps.executeUpdate();
                ps.close();
        ...
        int size = getBossForEverLog(bossid);
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
            ps = con.prepareStatement("UPDATE bossforeverlog SET size = ? Where characterid = ? and bossid = ?");
    ...
    public int getMaster(int aid) {
        int nx = 0;
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            int cid = aid;
            ResultSet rs;
            try (PreparedStatement limitCheck = con.prepareStatement("SELECT * FROM master WHERE apprentice=" + cid + "")) {
                rs = limitCheck.executeQuery();
                if (rs.next()) {
                    nx = rs.getInt("characterid");
                }
            }
            rs.close();
        } catch (SQLException e) {
    ...
    public boolean isMasterAndApprentice(int log) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            int ret_count = 0;
            boolean ret = false;
            PreparedStatement ps;
            ps = con.prepareStatement("select count(*) from master where characterid = ? and apprentice = ?");
            ps.setInt(1, id);
            ps.setInt(2, log);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public void deleteMasterAndApprentice(int log) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps = con.prepareStatement("DELETE FROM master WHERE characterid = ? and apprentice = ?");
            ps.setInt(1, id);
            ps.setInt(2, log);
            ps.executeUpdate();
            ps.close();
    ...
    public void deleteMasterAndApprentice(int id, int log) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps = con.prepareStatement("DELETE FROM master WHERE characterid = ? and apprentice = ?");
            ps.setInt(1, id);
            ps.setInt(2, log);
    ...
    public boolean isApprentice(int log) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            int ret_count = 0;
            boolean ret = false;
            PreparedStatement ps;
            ps = con.prepareStatement("select count(*) from master where apprentice = ?");
    ...
    public void setMasterAndApprentice(int log) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
            ps = con.prepareStatement("insert into master (characterid, apprentice) values (?,?)");
            ps.setInt(1, id);
            ps.setInt(2, log);
            ps.executeUpdate();
            ps.close();
    ...
    public String getApprenticeList() {
        int nx = 0;
        String name = "";
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            int cid = id;
            ResultSet rs;
            try (PreparedStatement limitCheck = con.prepareStatement("SELECT * FROM master WHERE characterid=" + cid + "")) {
                rs = limitCheck.executeQuery();
                if (rs.next()) {
                    nx = rs.getInt("apprentice");
                    name += getCharacterNameById(nx) + ",";
                }
            }
            rs.close();
            return name;
    ...
    public void updateBossForEverLog(String bossid, int size) {
        if (getBossForEverLog(bossid) <= -1) {
            try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
                PreparedStatement ps;
                ps = con.prepareStatement("insert into bossforeverlog (characterid, bossid) values (?,?)");
    ...
    public void updateAcForEverLog(String bossid, int size) {
        if (getAcForEverLog(bossid) <= -1) {
            try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
                PreparedStatement ps;
                ps = con.prepareStatement("insert into acforeverlog (accid, bossid) values (?,?)");
    ...
    public void addFastMove() {
        FastMove++;
    }

    public int getFastMove() {
        return FastMove;
    }
    ...
    public void setzb(int nx) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
            ps = con.prepareStatement("UPDATE accounts SET money = ? Where id = ?");
            ps.setInt(1, nx);
            ps.setInt(2, getClient().getAccID());
            ps.execute();
            ps.close();
    ...
    public void setmoneyb(int nx) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
            ps = con.prepareStatement("UPDATE accounts SET moneyb = ? Where id = ?");
            ps.setInt(1, nx);
            ps.setInt(2, getClient().getAccID());
            ps.execute();
            ps.close();
    ...
    public void modifymoney(int type, int quantity, boolean show) {
        switch (type) {
            ...
            case 2:
                ...
                setmoneyb(nxb + quantity);
                break;
        }
        if ((show) && (quantity != 0)) {
            dropMessage(5, "你已经 " + (quantity > 0 ? "获得 " : "使用 ") + quantity + (type == 1 ? " 钻石." : " 余额."));
        }
    }

    public void gainmoneyb(int slot) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            modifymoney(2, slot, true);
            PreparedStatement ps = con.prepareStatement("UPDATE 余额记录表 SET 消费记录 =消费记录- " + slot + " WHERE charID = " + this.id);
            ps.executeUpdate();
            ps.close();
        } catch (SQLException e) {
            FileoutputUtil.outputFileError(FileoutputUtil.MysqlEx_Log, e);
    ...
    public int 获取上限等级() {
        return GameConstants.等级上限;
    }

    public int 获取提交数量() {

        return GameConstants.提交数量;
    }

    public void 提交物品(int num) {
        GameConstants.提交数量 += num;
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps = con.prepareStatement("update 等级上限 set 等级=? ,提交数量=? ");
            ps.setInt(1, GameConstants.等级上限);
            ps.setInt(2, GameConstants.提交数量);
            ps.executeUpdate();
            ps.close();
            con.close();
        } catch (SQLException ex) {
    ...
    public void 突破等级() throws SQLException {
        GameConstants.等级上限 += 1;
        GameConstants.提交数量 = 0;
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps = con.prepareStatement("update 等级上限 set 等级=? ,提交数量=0 ");
            ps.setInt(1, GameConstants.等级上限);
            ps.executeUpdate();
            ps.close();
            con.close();
    ...
    public int getOneTimeLog(String bossid) {
        int ret_count = 0;
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps = con.prepareStatement("select count(*) from onetimelog where characterid = ? and log = ?");
            ps.setInt(1, this.id);
            ps.setString(2, bossid);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public void setOneTimeLog(String bossid) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps = con.prepareStatement("insert into onetimelog (characterid, log) values (?,?)");
            ps.setInt(1, this.id);
            ps.setString(2, bossid);
            ps.executeUpdate();
            ps.close();
    ...
    public void 置在线时间(long 在线时间) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps;
            ps = con.prepareStatement("Update characters set todayOnlineTime = ? Where id = ?");
            ps.setLong(1, 在线时间);
            ps.setInt(2, getId());
            ps.execute();
            ps.close();
    ...
    public void toggleTestingDPM() {
        this.testingdpm = !testingdpm;
    }

    public boolean isTestingDPM() {
        return testingdpm;
    }
    
    public void UpdateCash() {//点卷抵用存档刷新
        getClient().getSession().write(CWvsContext.updateMaplePoint(this));
    }
    ...
    */
    public int getBossLoga(String boss) {
        return getBossLoga(boss, 0);
    }

    public int getBossLoga(String boss, int type) {

        int count = 0;

        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps = con.prepareStatement("SELECT * FROM bossloga WHERE characterid = ? AND bossid = ?");
            ps.setInt(1, this.accountid);
            ps.setString(2, boss);
            ResultSet rs = ps.executeQuery();
            Calendar cal;
            if (rs.next()) {
                ...
                if (type == 0) {
                    if (bossTime != null) {
                        ...
                        if ((cal.get(5) + 1 <= Calendar.getInstance().get(5)) || (cal.get(2) + 1 <= Calendar.getInstance().get(2))) {
                            count = 0;
                            ps = con.prepareStatement("UPDATE bossloga SET count = 0  WHERE characterid = ? AND bossid = ?");
                            ps.setInt(1, this.id);
                            ps.setString(2, boss);
                            ps.executeUpdate();
                        }
                    ...
                    rs.close();
                    ps.close();
                    ps = con.prepareStatement("UPDATE bossloga SET time = CURRENT_TIMESTAMP() WHERE characterid = ? AND bossid = ?");
                    ps.setInt(1, this.id);
                    ps.setString(2, boss);
                    ps.executeUpdate();
                }
            } else {
                PreparedStatement psu = con.prepareStatement("INSERT INTO bossloga (characterid, bossid, count, type) VALUES (?, ?, ?, ?)");
       ...
       public void setBossLoga(String boss) {
        setBossLoga(boss, 0);
    }

    public void setBossLoga(String boss, int type) {
        setBossLoga(boss, type, 1);
    }

    public void setBossLoga(String boss, int type, int count) {
        int bossCount = getBossLog(boss, type);
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps = con.prepareStatement("UPDATE bossloga SET count = ?, type = ?, time = CURRENT_TIMESTAMP() WHERE characterid = ? AND bossid = ?");
            ps.setInt(1, bossCount + count);
            ps.setInt(2, type);
            ps.setInt(3, this.id);
            ps.setString(4, boss);
    ...
    public void resetBossLoga(String boss) {
        resetBossLoga(boss, 0);
    }

    public void resetBossLoga(String boss, int type) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps = con.prepareStatement("UPDATE bossloga SET count = ?, type = ?, time = CURRENT_TIMESTAMP() WHERE characterid = ? AND bossid = ?");
            ps.setInt(1, 0);
            ps.setInt(2, type);
	...
	public final long getLastOfflineTime() {
        ...
        public int getFZ1(int someid, String log1) {
        int jf = 0;
        try {
            Connection con = DBConPool.getInstance().getDataSource().getConnection();
            PreparedStatement ps = con.prepareStatement("select * from fz1 where cid = ? AND log = ?");
            ps.setInt(1, someid);
            ps.setString(2, log1);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
               jf = rs.getInt("sz1");
                ...
                PreparedStatement psu = con.prepareStatement("insert into fz1 (cid,log, sz1) VALUES (?, ?, ?)");
                psu.setInt(1, someid);
            ...
            System.err.println("FZ1读取发生错误: " + ex);
        }
        return jf;
    }
    
  public void setFZ1(int someid, String log1 ,int slot) {
        try {
            Connection con = DBConPool.getInstance().getDataSource().getConnection();
            PreparedStatement ps = con.prepareStatement("UPDATE fz1 SET sz1 = ? where cid = ? AND log = ?");
            ps.setInt(1, slot);
            ps.setInt(2, someid);
            ps.setString(3, log1);
            ps.executeUpdate();
            ...
            System.err.println("FZ1加减发生错误: " + ex);
        }
    } 
}
