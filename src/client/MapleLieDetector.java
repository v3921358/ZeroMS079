                ...
                if (((!MapleLieDetector.this.isPassed()) /*&& (!isItem)*/) && (searchchr != null)) {
                    if (MapleLieDetector.this.attempt >= 3) {
                        ...
                        if ((search_chr != null) && (search_chr.getId() != searchchr.getId())) {
                            search_chr.dropMessage(5, searchchr.getName() + " 未通过测谎仪检测。");
                        ...
                        World.Broadcast.broadcastGMMessage(CWvsContext.broadcastMsg(6, "[系统警告] 玩家: " + searchchr.getName() + " (等级 " + searchchr.getLevel() + ") 未通过测谎仪检测。"));
                    } else {
