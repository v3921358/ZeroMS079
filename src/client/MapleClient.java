...
public class MapleClient implements Serializable {
    ...
    ;
    public static final int DEFAULT_CARDS_CHARSLOT = 3;
    ...
    private int charscardslots = DEFAULT_CARDS_CHARSLOT;
    ...
    private static final Logger logger = LogManager.getLogger(MapleClient.class);
    ...
    /**
     * 加载此服务器下的所有角色，服务器指：蓝蜗牛，青鳄鱼，红螃蟹等。
     * @param serverId  服务器ID，从0开始
     * @return
     */
    ...
    public final byte getLoginState() { // TODO hide?
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            System.err.println(ex);
            loggedIn = false;
            throw new DatabaseException("error getting login state", ex);
        }
    }
        ...
        try {
            ...
            if (player.getMap() != null) {
              ...
              //  player.getMap().removePlayer(player);
                             if(ServerConfig.Offlinehangup){//离线挂机开关
					player.getMap().removePlayer2(player);
                            System.out.println("[离线挂机][名字:" + player.getName() + "][等级:" + player.getLevel() + "][IP:" + player.getClient().getSession().getRemoteAddress().toString().split(":")[0] + "] 地图："+player.getMap().getMapName());   
				}else{
					player.getMap().removePlayer(player);
                             System.out.println("[正常下线][名字:" + player.getName() + "][等级:" + player.getLevel() + "][IP:" + player.getClient().getSession().getRemoteAddress().toString().split(":")[0] + "] 地图："+player.getMap().getMapName());  
				}
                ...
                try {
                    ...
                    if (RemoveInChannelServer && ch != null) {
                        ch.removePlayer(idz, namez);//这个地方是清理角色的信息
                            ...
                            ZeroMS_UI.getInstance().updatePlayerList(player, false);
                        }
                    }
                    player = null;
                }
            ...
            public void run() {
                try {
                    if (getLatency() < 0) {
                        ...
                        if (getSession().getChannel().isActive()) {
                            getSession().close();
                        }
                    }
    ...
    public int getCharacterCardsSlots() {
        if (isGm()) {
            return 9;
        }
        if (charscardslots != DEFAULT_CARDS_CHARSLOT) {
            return charscardslots; //save a sql
        }
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            try (PreparedStatement ps = con.prepareStatement("SELECT * FROM character_cards_slots WHERE accid = ? AND worldid = ?")) {
                ps.setInt(1, accId);
                ps.setInt(2, world);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        charscardslots = rs.getInt("charscardslots");
                    } else {
                        try (PreparedStatement psu = con.prepareStatement("INSERT INTO character_cards_slots (accid, worldid, charscardslots) VALUES (?, ?, ?)")) {
                            psu.setInt(1, accId);
                            psu.setInt(2, world);
                            psu.setInt(3, charscardslots);
                            psu.executeUpdate();
                        }
                    }
                }
        ...
        return charscardslots;
    }

    public boolean gainCharacterCardsSlots() {
        if (getCharacterSlots() >= 9) {
            return false;
        }
        charscardslots++;
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            try (PreparedStatement ps = con.prepareStatement("UPDATE character_cards_slots SET charscardslots = ? WHERE worldid = ? AND accid = ?")) {
                ps.setInt(1, charscardslots);
                ps.setInt(2, world);
                ps.setInt(3, accId);
                ps.executeUpdate();
                ps.close();
    ...
    public final Timestamp getCreated() { // TODO hide?
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            FileoutputUtil.outputFileError(FileoutputUtil.MysqlEx_Log, ex);
            System.err.println(ex);
            throw new DatabaseException("error getting create", ex);
        }
    }
    ...
    public void setLoginKey(int key) {
        this.loginKey = key;
    }
    ...
    public final void updateLoginKey(int loginkey, int accountId) {
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps = con.prepareStatement("UPDATE `accounts` SET `loginkey` = ? WHERE id = ?");
            ps.setInt(1, loginkey);
