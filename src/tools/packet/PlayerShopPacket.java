...
public class PlayerShopPacket {
    ...
    public static final byte[] shopChat(final String message, final int slot) {
        ...
        mplew.writeShort(SendPacketOpcode.PLAYER_INTERACTION.getValue());
        mplew.write(PlayerInteractionHandler.Interaction.CHAT.action);//was15
        mplew.write(9);
        mplew.write(slot);
...
//        mplew.write(19);//was10
    ...
    public static final byte[] getTradePartnerAdd(final MapleCharacter chr, final int slot) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//        mplew.write(19);//was10
    ...
    public static final byte[] merchItemStore_ItemData(final MerchItemPackage pack) {
        ...
        mplew.write(0);
        mplew.writeInt(0);
        mplew.writeInt((int) pack.getMesos());
        mplew.write(0);
    ...
    public static byte[] getMiniGameExitAfter(boolean ready) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.PLAYER_INTERACTION.getValue());
        mplew.write(ready ? PlayerInteractionHandler.Interaction.EXIT_AFTER_GAME.action : PlayerInteractionHandler.Interaction.CANCEL_EXIT.action);
        return mplew.getPacket();
    }
    ...
    public static byte[] getMatchCardSelect(int turn, int slot, int firstslot, int type) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.PLAYER_INTERACTION.getValue());
        mplew.write(PlayerInteractionHandler.Interaction.SELECT_CARD.action);
        mplew.write(turn);
        mplew.write(slot);
        if (turn == 0) {
