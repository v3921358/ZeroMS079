...
public class CField {
    ...
    public static byte[] getServerIP(MapleClient c, String inetAddr, int port, int clientId) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.SERVER_IP.getValue());
        mplew.writeShort(0);
        byte[] ip = {(byte) 221, (byte) 231, (byte) 130, (byte) 70};
        try {
            ip = InetAddress.getByName(inetAddr).getAddress();
    ...
    public static byte[] getChannelChange(MapleClient c, String inetAddr, int port) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] getMacros0() {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.SKILL_MACRO.getValue());

        mplew.write(5); // number of macros
        for (int i = 0; i < 5; i++) {
    ...
    public static boolean timekey = false;
    ...
    public static byte[] MapEff(String path) {
        return environmentChange(path, 3);//was 3
    }
    ...
    public static byte[] setBoatState(int type) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//        }
        ...
        int energybuff = chr.getBuffSource(MapleBuffStat.ENERGY_CHARGE);
        int energyLevel = chr.getBuffedValue(MapleBuffStat.ENERGY_CHARGE) != null ? chr.getBuffedValue(MapleBuffStat.ENERGY_CHARGE) : 0;
        mplew.writeInt(energyLevel >= 10000 ? energybuff : 0);
        mplew.writeInt(Math.min(energyLevel, 10000));
        ...
        if (GameConstants.getInventoryType(chr.getChair()) == MapleInventoryType.SETUP && chr.getChair() > 0) {
            int chairid = chr.getChair();
            int tamingMob = MapleItemInformationProvider.getInstance().getChairMountId(chairid) - 50000;
            if (tamingMob <= 0) {
                tamingMob = 0;
            }
            if (tamingMob > 0) {
    ...
    public static byte[] PVPdamagePlayer(int chrId, int type, int monsteridfrom, int damage) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] spawnKite(int oid, int itemid, String name, String msg, Point pos, int ft) {
        ...
        mplew.writeInt(oid); // not sure
        mplew.writeInt(itemid);
        mplew.writeMapleAsciiString(msg);
        mplew.writeMapleAsciiString(name); // owner name
    ...
    public final static int[] SecondaryStatRemote = new int[GameConstants.MAX_BUFFSTAT];//new int[GameConstants.MAX_BUFFSTAT];
    ...
    static {
        ...
        SecondaryStatRemote[MapleBuffStat.DEFAULT_BUFFSTAT2.getPosition(true)] |= MapleBuffStat.DEFAULT_BUFFSTAT2.getValue();
    ...
    public static class InteractionPacket {
        ...
        public static byte[] getTradeInvite(MapleCharacter c, boolean cs) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//            mplew.writeInt(c.getLevel());
        ...
        public static byte[] getTradeStart(MapleClient c, MapleTrade trade, byte number, boolean cs) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//                mplew.writeInt(Integer.MAX_VALUE);
        ...
        public static byte[] getNPCTalkStyle(int npc, String talk, int[] args, boolean second, int itemid) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        public static byte[] getNPCTalkText(int npc, String talk) {
            ...
            mplew.writeShort(2); //3 regular 6 quiz
            mplew.writeMapleAsciiString(talk);
            mplew.writeInt(0);
            mplew.writeInt(0);
    ...
    public static class EffectPacket {
        ...
        public static byte[] showItemLevelupEffect() {
            return showForeignEffect(17);
        }

        public static byte[] showForeignItemLevelupEffect(int cid) {
            return showForeignEffect(cid, 17);
        }
    ...
    public static byte[] makerResult(boolean success, int itemMade, int itemCount, int mesos, List<Pair<Integer, Integer>> itemsLost, int catalystID, List<Integer> INCBuffGems) {
        ...
        mplew.writeShort(SendPacketOpcode.MAKER_SKILL.getValue());
